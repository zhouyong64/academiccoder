...
class OfflineGridUBytePicture : public Picture {
public:
  short int xOffset;
  short int yOffset;
  short int xSize;
  short int ySize;
  vector<unsigned char> bitmap; //nInputFeatures*ySize*xSize (row major order)
  void codifyInputData (SparseCnnInterface &interface);
  Picture* distort ();

  OfflineGridUBytePicture(int xSize, int ySize, int label_ = -1) : xSize(xSize), ySize(ySize) {
    label=label_;
    xOffset=-xSize/2;
    yOffset=-ySize/2;
  ...
  unsigned char interpolate(FloatPoint& p, int i) {
    return bilinearInterpolationScaled<unsigned char>
...
void OfflineGridUBytePicture::codifyInputData (SparseCnnInterface &interface) {
  for  (int i=0; i<nInputFeatures; i++)
...
//   OfflineGridUBytePicture* pic=new OfflineGridUBytePicture(*this);
//   RNG rng;
//   pic->jiggle(rng,2);
...
// void OfflineGridUByte_test_show_errors() {
//   RNG rng;
//   int mistakes=0;
...
//         dynamic_cast<OfflineGridUBytePicture*>(testCharacters[i])->drawPGM(filename);
//       }
//     }
//     delete batch;
//   }
