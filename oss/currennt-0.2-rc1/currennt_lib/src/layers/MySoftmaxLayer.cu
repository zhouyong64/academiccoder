...
namespace {
	...
	struct CalculateIdxCountFn
    {
        ...
        const real_t *maxIdxPerPat;

        __host__ __device__ void operator() (const int &seqIdx) const
        {
        	int* counts = new int[layerSize];
        	int curLen = 0;
        	int patIdx = 0;
            for (int i = 0; i < maxSeqLen; ++i) {
               patIdx = maxSeqLen * seqIdx + i;
               if (patTypes[patIdx] == PATTYPE_NONE)
               			break;
               curLen++;
               counts[(int)(maxIdxPerPat[patIdx])]++;
            }
            patIdx = maxSeqLen * seqIdx;
            real_t *offOutputs = &outputs[patIdx * layerSize];

			for (int j = 0; j < curLen; ++j) {
	            for (int i = 0; i < layerSize; ++i) {
	            		offOutputs[j * layerSize + i] = 1.0*counts[i]/curLen;
	            }
	        }
	    delete counts;
            
            /*
            thrust::transform(counts.begin(), counts.end(), thrust::constant_iterator<int>(curLen), counts.begin(),
                     thrust::divides<float>());
            thrust::copy(counts.begin(), counts.end(), 
            */
        }
    };
    
	struct CalculateMaxIdxFn
    {
        int layerSize;
...
namespace layers {
    ...
    void SoftmaxLayer<TDevice, TFfActFn>::computeForwardPass()
    {
        ...
        {{
            internal::CalculateIdxCountFn fn;
            fn.layerSize = this->size();
            fn.outputs   = helpers::getRawPointer(this->_outputs());
            fn.patTypes  = helpers::getRawPointer(this->patTypes());
			fn.maxSeqLen = this->curMaxSeqLength();
			fn.maxIdxPerPat      = helpers::getRawPointer(m_patTmp);
