...
def applyBn(rollingAverageForBatchNormalizationOverThatManyBatches, inputTrain, inputVal, inputTest, inputShapeTrain) :
    ...
    #for rolling average:
    muBnsArrayForRollingAverage = theano.shared(np.zeros( (rollingAverageForBatchNormalizationOverThatManyBatches, \
                                                           numberOfChannels), dtype = 'float32' ), borrow=True)
    varBnsArrayForRollingAverage = theano.shared(np.ones( (rollingAverageForBatchNormalizationOverThatManyBatches, \
                                                           numberOfChannels), dtype = 'float32' ), borrow=True)
...
class ConvLayer(Block):
    ...
    def _processInputWithBnNonLinearityDropoutPooling(self,
                ...
                rollingAverageForBatchNormalizationOverThatManyBatches, #If this is <= 0, we are not using BatchNormalization, 
        ...
        if useBnFlag and rollingAverageForBatchNormalizationOverThatManyBatches > 0 :
            ...
            self._newMu_B,
            self._newVar_B
            ) = applyBn( rollingAverageForBatchNormalizationOverThatManyBatches, inputToLayerTrain, inputToLayerVal, \
    ...
    def makeLayer(self,
                ...
                useBnFlag, # Must be true to do BN. Used to not allow doing BN on first layers straight on image, 
                #even if rollingAvForBnOverThayManyBatches > 0.
                rollingAverageForBatchNormalizationOverThatManyBatches, #If this is <= 0, we are not using \
