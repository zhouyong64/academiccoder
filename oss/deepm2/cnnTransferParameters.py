...
def transferParametersBetweenModels(myLogger, cnnTarget, cnnSource, listOfLayersToTransfer):
    ...
    cnnTarget : An instance of deepmedic.cnn3d.Cnn3d. This is the model that will receive \
    ...
    listOfLayersToTransfer: A list of integers. The integers are the depth of the layers of cnnTarget that \
    will be adopted from the pre-trained model.
        ...
        Example: In the original deepmedic, with 8 layers at each parallel path followed by 3 FC layers, \
        [1,2,3,4,9,10] will transfer parameters of the 4 first layers of EACH parallel pathway, \
        and 2 hidden FC layers (depth 9 and 10).
    
    Important Note on functionality:
    The two models can be of different depth. But each pathway of cnnSource must have at least as many layers as \
    the amount requested to transfer via listOfLayersToTransfer.
    ...
    If cnnTarget has more FMs than cnnSource, the FMs of the latter will be transfered to the FMs of the former \
    with the smallest indexes and the rest will remain unchanged.
    Number of parallel pathways between the models can differ. If cnnTarget has more pathways than cnnSource, \
    then the lower scale pathways receive weights from the lowest scale pathway availble.
    """
    # Classif layer. NOT Softmax layer, which is not registered in the FC path.
    depthDeepestLayerTarget = len(cnnTarget.pathways[0].getLayers()) + len(cnnTarget.getFcPathway().getLayers()) 
    ...
    for pathTarget_i in xrange( len(cnnTarget.pathways) ):
        ...
        for layerTarget_i in xrange( len(layersPathTarget) ):
            ...
            depthLayerTarget = layerTarget_i + 1 if typePathTarget <> pt.FC else layerTarget_i + 1 + \
                len( cnnTarget.pathways[0].getLayers() ) # +1 cause indexes. index 0 is depth 1.
            
            # Check if this layer of Target should receive parameters from Source.
            boolTransferLayer = False
            # For list == None, we do the default transfer. Transfer all except the deepest classif Layer.
            if listOfLayersToTransfer is None and depthLayerTarget <> depthDeepestLayerTarget: 
            ...
            if boolTransferLayer:
                myLogger.print3("[Pathway_" + str(pathTarget.getStringType()) + "][Conv.Layer_" + str(layerTarget_i) + \
                                " (index)], depth [" + str(depthLayerTarget) + "] (Target): Receiving parameters...")
                # transfer stuff.
                # Get the correct Source path.
                if typePathTarget <> pt.FC:
                    # if cnnSource has at least as many parallel pathways (-1 to exclude FC) as 
                    # the number of the current pathwayTarget (+1 because it's index).
                    if (len(cnnSource.pathways) - 1) >= (pathTarget_i + 1) : 
                        ...
                        myLogger.print3("\t Source model has less parallel paths than Target. "+\
                                        "Parameters of Target are received from last parallel path of Source [Pathway_" + \
                ...
                if len(pathSource.getLayers()) < layerTarget_i + 1:
                    myLogger.print3("ERROR: This [Pathway_" + str(pathTarget.getStringType()) + \
                                    "] of the [Source] model was found to have less layers than required!\n"+\
                                    ...
                                    "\t Tried to transfer parameters to [Target] layer with *index* in this pathway: [" + \
                                    str(layerTarget_i) + "]. (specified depth [" + str(depthLayerTarget) + "]).\n"+\
                                    "\t Note: First layer of pathway has *index* [0].\n"+\
                                    "\t Note#2: To transfer parameters from a Source model with less layers than the Target, \
                                    specify the depth of layers to transfer using the command line option [-layers].\n"+\
                                    "\t Try [-h] for help or see documentation.\n"+\
                                    "Exiting!"); exit(1)
                
                layerSource_i = layerTarget_i
                layerSource = pathSource.getLayers()[layerSource_i]
                myLogger.print3("\t ...receiving parameters from [Pathway_" + str(pathSource.getStringType()) + \
                                "][Conv.Layer_" + str(layerSource_i) + " (index)] (Source).")
                ...
                # It's the last Classification layer that was transfered. 
                # Also transfer the biases of the Softmax layer, which is not in FC path.
                if depthLayerTarget == depthDeepestLayerTarget: 
                    myLogger.print3("\t Last Classification layer was transfered. Thus for completeness, \
                        transfer the biases applied by the Softmax pseudo-layers. ")
                    ...
                    transferParametersBetweenLayers(myLogger=myLogger, layerTarget = softMaxLayerTarget, \
...
def transferParametersBetweenLayers(myLogger, layerTarget, layerSource ):
    ...
    if layerTarget._W is not None and layerSource._W is not None:
        ...
        targetValue[:minNumFmsInLayers, :minNumOfChansInInput, :,:,:] = sourceValue[:minNumFmsInLayers, \
    ...
    minLengthRollAvForBn = min( layerTarget._rollingAverageForBatchNormalizationOverThatManyBatches, \
    ...
    if layerTarget._muBnsArrayForRollingAverage is not None and layerSource._muBnsArrayForRollingAverage is not None:
        ...
        targetValue[:minLengthRollAvForBn, :minNumOfChansInInput] = \
            sourceValue[:minLengthRollAvForBn, :minNumOfChansInInput]
