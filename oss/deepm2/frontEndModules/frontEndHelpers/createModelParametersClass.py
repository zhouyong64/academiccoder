...
class CreateModelSessionParameters(object) :
    ...
    def errorSegmDimensionsSmallerThanReceptiveF(receptiveFieldNormal, segmentDimensions, tr0_val1_inf2) :
        ...
        The network was made with a receptive field of dimensions: ", receptiveFieldNormal, ". But in the case of :", \
        stringsTrainValInference[tr0_val1_inf2], " the dimensions of the input segment were specified smaller: ", \
        segmentDimensions, ". Please fix this by adjusting number of layer and kernel dimensions! Exiting!"; exit(1)
    @staticmethod
    def errorRequireNumberOfClasses() :
        print "ERROR: Number of classses not specified in the config file, which is required. Please specify in the \
        format: numberOfOutputClasses = 3 (any integer). This number should be including the background class! \
        For instance if the class is binary, set this to 2! Exiting!"; exit(1)
    errReqNumClasses = errorRequireNumberOfClasses
    
    
    @staticmethod
    def errorRequireNumberOfChannels() :
        print "ERROR: Parameter \"numberOfInputChannels\" not specified or specified smaller than 1. Please specify \
        the number of input channels that will be used as input to the CNN, in the format: numberOfInputChannels = \
        number (an integer > 0). Exiting!"; exit(1)
    ...
    def errorRequireFMsNormalPathwayGreaterThanNothing() :
        ...
        This parameter should be given in the format: numberFMsPerLayerNormal = \
        [number-of-FMs-layer1, ..., number-of-FMs-layer-N], where each number is an integer greater than zero. \
        It specifies the number of layers (specified by the number of entries in the list) and the number of \
        Feature Maps at each layer of the normal-scale pathway. Please provide and retry. Exiting!"; exit(1)                
    ...
    def errorRequireKernelDimensionsPerLayerNormal() :
        ...
        It should be provided in the format: kernelDimPerLayerNormal = \
        [ [dim1-of-kernels-in-layer-1, dim2-of-kernels-in-layer-1, dim3-of-kernels-in-layer-1], ..., \
        ...
        Each sublist should contain 3 integer ODD numbers greater than zero, \
        which should specify the dimensions of the 3-dimensional kernels. For instace: \
        kernelDimPerLayerNormal = [[5,5,5],[3,3,3]] for a pathway with 2 layers, \
        the first of which has 5x5x5 kernels and the second 3x3x3 kernels. Please fix and retry \n \
        WARN: The kernel dimensions should be ODD-NUMBERS. System was not thoroughly tested for kernels of \
        even dimensions! Exiting!"; exit(1)
    ...
    def errorRequireKernelDimensionsSubsampled(numFMsPerLayerNormal, numFMsPerLayerSubsampled) :
        print "ERROR: It was requested to use the 2-scale architecture, with a subsampled pathway. Because of \
        limitations to the developed system, the two pathways must have the save size of receptive field. By default, \
        if \"useSubsampledPathway\" = True, and the parameters \"numberFMsPerLayerSubsampled\" and \
        \"kernelDimPerLayerSubsampled\" are not specified, the second pathway will be constructed symmetrical to \
        the first. However, in this case, \"numberFMsPerLayerSubsampled\" was specified. It was found to have ", \
        len(numFMsPerLayerSubsampled)," entries, which specified this amount of layers in the subsampled pathway. \
        This is different than the number of layers in the Normal pathway, specified to be: ", \
        len(numFMsPerLayerNormal),". In this case, we require you to also provide the parameter \
        \"numberFMsPerLayerSubsampled\", specifying kernel dimensions in the subsampled pathway, in a fashion \
        that results in same size of receptive field as the normal pathway."
    ...
    def errorRequireKernelDimensionsPerLayerSubsampledCorrect() :
        ...
        It should be provided in the format: kernelDimPerLayerSubsampled = [ [dim1-of-kernels-in-layer-1, \
        dim2-of-kernels-in-layer-1, dim3-of-kernels-in-layer-1], ..., [dim1-of-kernels-in-layer-N, \
        dim2-of-kernels-in-layer-N, dim3-of-kernels-in-layer-N] ]. It is a list of sublists. \
        One sublist should be provided per layer of the SUBSAMPLED pathway. Thus it should have as many entries \
        as the entries in parameter \"numberFMsPerLayerSubsampled\". (WARN: if the latter is not provided, it is by \
        default taken equal to what specified for \"numberFMsPerLayerNormal\", in order to make the pathways \
        symmetrical). Each sublist should contain 3 integer ODD numbers greater than zero, which should specify \
        the dimensions of the 3-dimensional kernels. For instace: kernelDimPerLayerNormal = [[5,5,5],[3,3,3]] \
        for a pathway with 2 layers, the first of which has 5x5x5 kernels and the second 3x3x3 kernels. \
        Please fix and retry. (WARN: The kernel dimensions should be ODD-NUMBERS. System was not thoroughly \
        tested for kernels of even dimensions!)"
        CreateModelSessionParameters.warnForSameReceptiveField()
        print "Exiting!"; exit(1)
    ...
    def errorReceptiveFieldsOfNormalAndSubsampledDifferent(kernDimPerLayerNormal, receptiveFieldSubsampled) :
        print "ERROR: The receptive field of the normal pathway was calculated = ", len(kernDimPerLayerNormal), \
        " while the receptive field of the subsampled pathway was calculated=", len(receptiveFieldSubsampled), \
        ". Because of limitations to the developed system, the two pathways must have the save size of receptive field.\
         Please provide a combination of \"numberFMsPerLayerSubsampled\" and \"kernelDimPerLayerSubsampled\" that gives \
         the same size of field as the normal pathway. If unsure of how to proceed, please ommit specifying \
         \"numberFMsPerLayerSubsampled\" and \"kernelDimPerLayerSubsampled\" in the config file, and the second \
         subsampled pathway will be automatically created to mirror the normal. Else, if you want to just specify the \
         number of Feature Maps in the subsampled, provide \"numberFMsPerLayerSubsampled\" = \
         [num-FMs-layer1, ..., num-FMs-layerN], with N the same number as the normal pathway, and we will then use the \
         same kernel-sizes as the normal pathway. Exiting!"; exit(1)
    @staticmethod
    def errorReqInitializationMethod01() :
        print "ERROR: Parameter \"initializeClassic0orDelving1\" must be given equal to 0 or 1. \
        Omit for default (=1). Exiting!"; exit(1)
    @staticmethod
    ...
    def errReqSameNumOfLayersPerSubPathway():
        ...
        print "\tHowever currently this functionality requires that the same number of layers are used in \
        both pathways (limitation in the code)."
        print "\tUser specified in \"numberFMsPerLayerSubsampled\" sublists of different length. Each list \
        should have the same lenght, as many as the wanted number of layers. Please adress this."        
        print "Exiting!"; exit(1)
    
    @staticmethod
    def errorSubFactor3d() :
        print "ERROR: The parameter \"subsampleFactor\" must have 3 entries, one for each of the 3 dimensions. \
        Please provide it in the format: subsampleFactor = [subFactor-dim1, subFactor-dim2, subFactor-dim3]. \
    ...
    def errorRequireSegmentDimensionsTrain() :
        print "ERROR: The parameter \"segmentsDimTrain\" was is required but not given. It specifies the size of the \
        3D segment that is given as input to the network. It should be at least as large as the receptive field of \
        the network in each dimension. Please specify it in the format: segmentsDimTrain = [dim-1, dim-2, dim-3]. \
        Exiting!"; exit(1)
    errReqSegmDimTrain = errorRequireSegmentDimensionsTrain
    
    @staticmethod
    def errorRequireBatchSizeTrain() :
        print "ERROR: The parameter \"batchSizeTrain\" was not specified, although required. This parameter specifies \
        how many training-samples (segments) to use to form a batch, on which a single training iteration is performed. \
        The bigger the better, but larger batches add to the memory and computational burden. \
        Depending on the segment-size, the batch size should be smaller (if big segment sizes are used) or larger \
        (if small segment sizes are used). A number between 10 to 100 is suggested. Please specify in the format: \
        batchSizeTrain = 10 (a number). Exiting!"; exit(1)
    errReqBatchSizeTr = errorRequireBatchSizeTrain
    @staticmethod
    def errorResLayer1(strPathwayType) :
        print "ERROR: The parameter \"layersWithResidualConn\" for the [", strPathwayType, "] pathway was specified to \
        include the number 1, ie the 1st layer."
        print "\t This is not an acceptable value, as a residual connection is made between the output of the specified \
        layer and the input of the previous layer. There is no layer before the 1st!"
        print "\t Provide a list that does not iinclude the first layer, eg layersWithResidualConnNormal = [4,6,8], \
        or an empty list [] for no such connections. Exiting!"; exit(1)
        
    @staticmethod
    def warnForSameReceptiveField() :
        print "WARN: Because of limitations to the developed system, the two pathways must have the save size of \
        receptive field. If unsure of how to proceed, please ommit specifying \"numberFMsPerLayerSubsampled\" and \
        \"kernelDimPerLayerSubsampled\" in the config file, and the second subsampled pathway will be automatically \
        created to mirror the normal. Else, if you want to just specify the number of Feature Maps in the subsampled, \
        provide \"numberFMsPerLayerSubsampled\" = [num-FMs-layer1, ..., num-FMs-layerN], with N the same number as the \
        normal pathway, and we will then use the same kernel-sizes as the normal pathway."
    @staticmethod
    ...
    def changeDatastructureToListOfListsForSecondaryPathwaysIfNeeded(listFromConfig) :
        ...
        if not isinstance(listFromConfig, list) :
            print "ERROR: variable \"", listFromConfig, "\" given in modelConfig.cfg should be either a list of integers,\
             or a list of lists of integers, in case multiple lower-scale pathways are wanted. Please correct it. Exiting."; 
            exit(1)
        ...
        if not (allElementsAreLists or noElementIsList) : #some are lists and some are not
            ...
            #Seems ok, but the structure is not a list of lists. It's probably the old type, eg [3,3,3]. 
            #Lets change to list of lists.
    ...
    def __init__(   self,
        ...
        self.kernDimPerLayerNormal = kernDimNormal if checkKernDimPerLayerCorrect3dAndNumLayers(kernDimNormal, numOfLayers) \
                                                    else self.errReqKernDimNormal()
        # Just for COMPATIBILITY CHECKS!
        self.receptiveFieldNormal = \
            calcReceptiveFieldDims(self.kernDimPerLayerNormal) 
        ...
        if not self.useSubsampledBool :
            ...
            # check that all subsampled pathways have the same number of layers. Limitation in the code currently, 
            #because I use kernDimSubsampled for all of them.
            ...
            elif kernDimSubsampled == None and numOfLayersInEachSubPath <> len(self.numFMsPerLayerNormal) : 
            ...
            else : #kernel dimensions specified and are correct (3d, same number of layers as subsampled specified). 
                #Need to check the two receptive fields and make sure they are correct.
                self.kernDimPerLayerSubsampled = kernDimSubsampled
                self.receptiveFieldSubsampled = \
                    calcReceptiveFieldDims(\
                                                                                self.kernDimPerLayerSubsampled)
                if self.receptiveFieldNormal <> self.receptiveFieldSubsampled :
                    self.errorReceptiveFieldsOfNormalAndSubsampledDifferent(self.receptiveFieldNormal, \
                                                                            self.receptiveFieldSubsampled)
            ...
            for secondaryPathway_i in xrange(len(self.subsampleFactor)) : #It should now be a list of lists, one \
                #sublist per secondary pathway. This is what is currently defining how many pathways to use.
            ...
            # Default behaviour:
            # If less sublists in numFMsPerLayerSubsampled were given than numOfSubsPaths, add more sublists of \
            # subsampleFactors, for the extra subpaths.
            for _ in range( numOfSubsPaths - len(self.numFMsPerLayerSubsampled) ) :
                ...
                self.numFMsPerLayerSubsampled.append( [ max(1, int(numFmsInLayer_i/2)) for numFmsInLayer_i in \
                                            numFmsForLayersOfLastSubPath ] ) # half of how many previous subPath had.
            # If less sublists in subsampleFactor were given than numOfSubsPaths, add more sublists of \
            # subsampleFactors, for the extra subpaths.
            for _ in range( numOfSubsPaths - len(self.subsampleFactor) ) :
                ...
                self.subsampleFactor.append( [ subFactorInDim_i + 2 for subFactorInDim_i in self.subsampleFactor[-1] ] ) 
            
            # Residuals and lower ranks.
            residConnAtLayersSubsampled = residConnAtLayersSubsampled if residConnAtLayersSubsampled <> None else \
                residConnAtLayersNormal
            lowerRankLayersSubsampled = lowerRankLayersSubsampled if lowerRankLayersSubsampled <> None else \
                lowerRankLayersNormal
            
        #==FC Layers==
        self.numFMsInExtraFcs = numFMsFc if numFMsFc <> None else []
        self.kernelDimsFirstFcLayer = kernelDimsFirstFcLayer if kernelDimsFirstFcLayer <> None else [1,1,1]
        assert len(self.kernelDimsFirstFcLayer) == 3 and (False not in [ dim > 0 for dim in \
                                                                              self.kernelDimsFirstFcLayer] )
        ...
        for (tr0_val1_inf2, segmentDimensions) in [ (0,self.segmDimNormalTrain), (1,self.segmDimNormalVal), \
        ...
        #default = [0.0, 0.5, ..., 0.5]
        self.dropFc = dropFc if dropFc <> None else self.defaultDropFcList(self.numFMsInExtraFcs) 
        ...
        self.bnRollingAverOverThatManyBatches = bnRollingAverOverThatManyBatches if \
            bnRollingAverOverThatManyBatches <> None else 60
        
        #==============CALCULATED=====================
        # Residual Connections backwards, per pathway type :
        self.checkLayersForResidualsGivenDoNotInclude1st(residConnAtLayersNormal, residConnAtLayersSubsampled, \
                                                                                        residConnAtLayersFc)
        ...
        self.convLayersToConnectToFirstFcForMultiscaleFromAllLayerTypes = [ [], [] ] #a sublist for each pathway. 
        # Starts from 0 index. Give a sublist, even empty for no connections.
        ...
        #MP is applied >>AT THE INPUT of the layer<<. To use mp to a layer, put a list of [[dsr,dsc,dsz], [strr,strc,strz], 
        # [mirrorPad-r,-c,-z], mode] which give the dimensions of the mp window, the stride, how many times to mirror \
        # the last slot at each dimension for padding (give 0 for none), the mode (usually 'max' pool). \
        ...
        mpParamsNorm = [ [] for layeri in xrange(len(self.numFMsPerLayerNormal)) ] 
        # CAREFUL about the [0]. Only here till this structure is made different per pathway and not pathwayType.
        mpParamsSubs = [ [] for layeri in xrange(len(self.numFMsPerLayerSubsampled[0])) ] if self.useSubsampledBool else [] 
        mpParamsFc = [ [] for layeri in xrange(len(self.numFMsInExtraFcs) + 1) ] #FC. This should NEVER be used for segmentation. \
        #Possible for classification though.
        self.maxPoolingParamsStructure = [ mpParamsNorm, mpParamsSubs, mpParamsFc]
        #Higher temperatures make the probabilities LESS distinctable. Actions have more similar probabilities.
        self.softmaxTemperature = 1.0  
    ...
    def printParametersOfThisSession(self) :
        ...
        logPrint("Residual connections added at the output of layers (indices from 0) = " + \
                 str(self.indicesOfLayersToConnectResidualsInOutput[0]))
        logPrint("Layers that will be made of Lower Rank (indices from 0) = " + \
                 str(self.indicesOfLowerRankLayersPerPathway[0]))
        ...
        logPrint("Number of Layers (per sub-pathway) = " + str([ len(numFmsPerLayerForSubPath) for \
                                                        numFmsPerLayerForSubPath in self.numFMsPerLayerSubsampled ]) )
        ...
        logPrint("Residual connections added at the output of layers (indices from 0) = " + \
                                        str(self.indicesOfLayersToConnectResidualsInOutput[1]))
        logPrint("Layers that will be made of Lower Rank (indices from 0) = " + \
                                str(self.indicesOfLowerRankLayersPerPathway[1]))
        ...
        logPrint("Residual connections added at the output of layers (indices from 0) = " + \
                                        str(self.indicesOfLayersToConnectResidualsInOutput[2]))
        logPrint("Layers that will be made of Lower Rank (indices from 0) = " + \
                                        str(self.indicesOfLowerRankLayersPerPathway[2]))
        #logPrint("Parameters for pooling before convolutions in this pathway = " +  + str(self.maxPoolingParamsStructure[2]))
        logPrint("Dimensions of Kernels in the 1st FC layer (Classif. layer if no hidden FCs used) = " + \
                                        str(self.kernelDimsFirstFcLayer))
        ...
        logPrint("Drop.R. for each layer in FC Pathway (additional FC layers + Classific.Layer at end) = " + \
                 str(self.dropoutRatesForAllPathways[2]))
        
        logPrint("~~Weight Initialization~~")
        logPrint("Classic random N(0,0.01) initialization (0), or ala \"Delving Into Rectifier\" (1) = " + \
                 str(self.initialMethodClassic0Delving1))
        ...
        logPrint("Apply BN straight on pathways' inputs (eg straight on segments) = " + \
                                                    str(self.applyBnToInputOfPathways))
        logPrint("Batch Normalization uses a rolling average for inference, over this many batches = " + \
                                                                str(self.bnRollingAverOverThatManyBatches))
