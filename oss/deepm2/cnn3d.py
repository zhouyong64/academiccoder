...
def padImgWithMirror(inputImage, voxelsPerDimToPad) :
    ...
    paddedImage = T.concatenate([inputImage[:, :, int(voxelsPerDimToPad[0] / 2) - 1::-1 , :, :], \
                                 inputImage], axis=2) if padLeft > 0 else inputImage
    paddedImage = T.concatenate([paddedImage, paddedImage[ :, :, -1:-1 - int((voxelsPerDimToPad[0] + 1) / 2):-1, :, :]], \
                                axis=2) if padRight > 0 else paddedImage
    # c-axis
    padLeft = int(voxelsPerDimToPad[1] / 2); padRight = int((voxelsPerDimToPad[1] + 1) / 2);
    paddedImage = T.concatenate([paddedImage[:, :, :, padLeft - 1::-1 , :], paddedImage], axis=3) if \
                                                                            padLeft > 0 else paddedImage
    paddedImage = T.concatenate([paddedImage, paddedImage[:, :, :, -1:-1 - padRight:-1, :]], axis=3) if \
                                                                            padRight > 0 else paddedImage
    # z-axis
    padLeft = int(voxelsPerDimToPad[2] / 2); padRight = int((voxelsPerDimToPad[2] + 1) / 2)
    paddedImage = T.concatenate([paddedImage[:, :, :, :, padLeft - 1::-1 ], paddedImage], axis=4) if \
                                                                            padLeft > 0 else paddedImage
    paddedImage = T.concatenate([paddedImage, paddedImage[:, :, :, :, -1:-1 - padRight:-1]], axis=4) if \
                                                                            padRight > 0 else paddedImage
...
class Cnn3d(object):
    def __init__(self):
        ...
        # initial value, changed in make_cnn_model().compileTrainingFunction()
        ...
        # list of shared_variables. Each of the individual Dws is a sharedVar. This whole thing isnt.
        ...
        # the rolling average accumulator of the variance of the grad (grad^2)
        ...
        #Can also be pre-set by an existing tensor if required in future extensions.
    ...
    def change_learning_rate_of_a_cnn(self, newValueForLearningRate, myLogger=None) :
        stringToPrint = "UPDATE: (epoch-cnn-trained#" + str(self.numberOfEpochsTrained) + ") Changing the Cnn's Learning Rate to: " \
                                                                                            + str(newValueForLearningRate)
        if myLogger <> None :
    ...
    def change_momentum_of_a_cnn(self, newValueForMomentum, myLogger=None):
        stringToPrint = "UPDATE: (epoch-cnn-trained#" + str(self.numberOfEpochsTrained) + \
                            ") Changing the Cnn's Momentum to: " + str(newValueForMomentum)
        if myLogger <> None :
            myLogger.print3(stringToPrint)
        else :
            print stringToPrint
    ...
    def changeB1AndB2ParametersOfAdam(self, b1ParamForAdam, b2ParamForAdam, myLogger) :
        myLogger.print3("UPDATE: (epoch-cnn-trained#" + str(self.numberOfEpochsTrained) + \
                        ") Changing the Cnn's B1 and B2 parameters for ADAM optimization to: B1 = " + \
                        str(b1ParamForAdam) + " || B2 = " + str(b2ParamForAdam))
        self.b1_adam = b1ParamForAdam
        self.b2_adam = b2ParamForAdam
        
    def changeRhoParameterOfRmsProp(self, rhoParamForRmsProp, myLogger) :
        myLogger.print3("UPDATE: (epoch-cnn-trained#" + str(self.numberOfEpochsTrained) + \
                        ") Changing the Cnn's Rho parameter for RMSProp optimization to: Rho = " + str(rhoParamForRmsProp))
    ...
    def checkMeanValidationAccOfLastEpochAndUpdateCnnsTopAccAchievedIfNeeded(self,
        ...
        if meanValidationAccuracyOfLastEpoch > highestAchievedValidationAccuracyOfCnn + \
                                    minIncreaseInValidationAccuracyConsideredForLrSchedule :
            ...
            myLogger.print3("UPDATE: In this last epoch (cnnTrained) #" + \
                            str(self.topMeanValidationAccuracyAchievedInEpoch[1]) + \
                            " the CNN achieved a new highest mean validation accuracy of: " + \
                            str(self.topMeanValidationAccuracyAchievedInEpoch[0]))
            
    def _initializeSharedVarsForInputsTrain(self) :
        # ======= Initialize sharedVariables ==========
        self._initializedSharedVarsTrain = True
        # Create the needed shared variables. Number of dimensions should be correct (5 for x, 4 for y). 
        #But size is placeholder. Changes when shared.set_value during training.
        ...
        for subsPath_i in xrange(self.numSubsPaths) :
            self.sharedInpXPerSubsListTrain.append(theano.shared(np.zeros([1, 1, 1, 1, 1], dtype="float32"), \
                                                                 borrow=self.borrowFlag))
        # When storing data on the GPU it has to be stored as floats (floatX). Later this variable is cast as "int", 
        #to be used correctly in computations.
    ...
    def _initializeSharedVarsForInputsVal(self) :
        ...
        for subsPath_i in xrange(self.numSubsPaths) :
            self.sharedInpXPerSubsListVal.append(theano.shared(np.zeros([1, 1, 1, 1, 1], dtype="float32"), \
                                                               borrow=self.borrowFlag))
    ...
    def _initializeSharedVarsForInputsTest(self) :
        ...
        for subsPath_i in xrange(self.numSubsPaths) :
            self.sharedInpXPerSubsListTest.append(theano.shared(np.zeros([1, 1, 1, 1, 1], dtype="float32"), \
                                                                borrow=self.borrowFlag))
            
    def freeGpuTrainingData(self) :
        # False if this is called (eg save model) before train/val/test function is compiled.
        if self._initializedSharedVarsTrain :  
    ...
    # for inference with batch-normalization. Every training batch, 
    #this is called to update an internal matrix of each layer, with the last mus and vars, 
    ...
    def _initializeSharedVariablesOfOptimizer(self, myLogger) :
        ...
        paramsToOpt = self._getTrainableParameters(myLogger)
        if self.sgd0orAdam1orRmsProp2 == 0 :
            self._initializeSharedVariablesOfSgd(paramsToOpt)
        elif self.sgd0orAdam1orRmsProp2 == 1 :
            self._initializeSharedVariablesOfAdam(paramsToOpt)
        elif self.sgd0orAdam1orRmsProp2 == 2 :
            self._initializeSharedVariablesOfRmsProp(paramsToOpt)
        else :
            return False
        return True
        
    def _initializeSharedVariablesOfSgd(self, paramsToOpt) :
        self.velocities_forMom = []
        for param in paramsToOpt :
            v = theano.shared(param.get_value() * 0., broadcastable=param.broadcastable)
            self.velocities_forMom.append(v)
            
    def getUpdatesAccordingToSgd(self, cost, paramsToOpt) :
        # create a list of gradients for all model parameters
        grads = T.grad(cost, paramsToOpt)
        
        updates = []
        # The below will be 1 if nonNormalized momentum, and (1-momentum) if I am using normalized momentum.
        multiplierForCurrentGradUpdateForNonNormalizedOrNormalizedMomentum = 1.0 - \
                        self.momentum * self.momentumTypeNONNormalized0orNormalized1
        
        for param, grad, v in zip(paramsToOpt, grads, self.velocities_forMom) :
    ...
    def _initializeSharedVariablesOfRmsProp(self, paramsToOpt) :
        self.accuGradSquare_listForAllParamsRmsProp = []
        self.velocities_forMom = []
        for param in paramsToOpt :
            # accumulates the mean of the grad's square.
    ...
    def getUpdatesAccordingToRmsProp(self, cost, params) :
        ...
        for param, grad, accu, v in zip(params, grads, self.accuGradSquare_listForAllParamsRmsProp, self.velocities_forMom):
            ...
            stepToGradientDirection = multiplierForCurrentGradUpdateForNonNormalizedOrNormalizedMomentum * \
                                    (self.learning_rate * grad / T.sqrt(accu_new + self.epsilonForRmsProp))
            newVelocity = self.momentum * v - stepToGradientDirection
    ...
    def _initializeSharedVariablesOfAdam(self, paramsToOpt) :
    ...
    def _getUpdatesOfTrainableParameters(self, myLogger, cost) :
        ...
        paramsToOpt = self._getTrainableParameters(myLogger)
        if self.sgd0orAdam1orRmsProp2 == 0 :
            myLogger.print3("Optimizer used: [SGD]. Momentum used: Classic0 or Nesterov1 : " + \
                                                        str(self.classicMomentum0OrNesterov1))
            updates = self.getUpdatesAccordingToSgd(cost, paramsToOpt)
        elif self.sgd0orAdam1orRmsProp2 == 1 :
            myLogger.print3("Optimizer used: [ADAM]. No momentum implemented for Adam.")
            updates = self.getUpdatesAccordingToAdam(cost, paramsToOpt)
        elif self.sgd0orAdam1orRmsProp2 == 2 :
            myLogger.print3("Optimizer used: [RMSProp]. Momentum used: Classic0 or Nesterov1 : " + \
                                                            str(self.classicMomentum0OrNesterov1))
            updates = self.getUpdatesAccordingToRmsProp(cost, paramsToOpt)
        return updates
    
    def _getTrainableParameters(self, myLogger):
        # A getter. Don't alter anything here!
        paramsToOpt = []  # Ws and Bs
        for pathway in self.pathways :
            for layer_i in xrange(0, len(pathway.getLayers())) :
                if layer_i not in self.indicesOfLayersPerPathwayTypeToFreeze[ pathway.pType() ] :
                    paramsToOpt = paramsToOpt + pathway.getLayer(layer_i).getTrainableParams()
                else : # Layer will be held fixed. Notice that Batch Norm parameters are still learnt.
                    myLogger.print3("WARN: [Pathway_" + str(pathway.getStringType()) + \
                                    "] The weights of [Layer-"+str(layer_i)+\
    ...
    # However, if I need to use a pretrained model, and train it in a second stage, 
    #I should recall this, with the new stage's parameters, and then recompile trainFunction.
    ...
    def _getUpdatesForBnRollingAverage(self) :
        ...
        # ... and here we update the sharedVariable which is used "from the outside during do_training()" to 
        #update the rolling-average-matrix for inference. Do for all layers.
        updatesForBnRollingAverage = []
        for pathway in self.pathways :
            for layer in pathway.getLayers() :
                #CAREFUL: WARN, PROBLEM, THEANO BUG! If a layer has only 1FM, the .newMu_B ends up being of type (true,) 
                #instead of vector!!! Error!!!
    ...
    def compileTrainFunction(self, myLogger) :
        ...
        # 2. Build model here. Which internally LOADS the weights, array made by newModel. Dont initialize a model here. 
        #If you want to pretrain, have a -pretrainedModel function to create a new model.
        # 3. initializeTrainingState() if the input variable (1) says so (eg to do another training stage). 
        #Otherwise, dont call it, to resume training.
        myLogger.print3("...Building the training function...")
        
        if not self.checkTrainingStateAttributesInitialized() :
            myLogger.print3("ERROR: Prior to compiling the training function, training state \
                attributes need to be initialized via a call of [Cnn3d.setTrainingStateAttributes(...)]. Exiting!"); exit(1)
        ...
        # When storing data on the GPU it has to be stored as floats (floatX). Thus the sharedVariable is FloatX/32. 
        #Here this variable is cast as "int", to be used correctly in computations.
        intCastSharedLabelsYTrain = T.cast(self.sharedLabelsYTrain, 'int32')
        # These two were added to counter class imbalance by changing the weights in the cost function
        inputVectorWeightsOfClassesInCostFunction = T.fvector()  
        ...
        if self.costFunctionLetter == "L" :
            ...
            myLogger.print3("ERROR: Problem in make_cnn_model(). The parameter self.costFunctionLetter did not \
                    have an acceptable value( L,D,J ). Exiting."); exit(1)
            
        cost = (costFromLastLayer + self.L1_reg_constant * self._getL1RegCost()
    ...
    def compileValidationFunction(self, myLogger) :
        ...
        for subPath_i in xrange(self.numSubsPaths) : # if there are subsampled paths...
            ...
            givensSet.update({ xSub: sharedInpXSubVal[index * self.batchSizeValidation: (index + 1) * \
                                                                        self.batchSizeValidation] })
        givensSet.update({ y: intCastSharedLabelsYVal[index * self.batchSizeValidation: (index + 1) * \
                                                                        self.batchSizeValidation] })
    ...
    def compileTestAndVisualisationFunction(self, myLogger) :
        ...
        funcList_AllFmActs_Preds = []
        for pathway in self.pathways :
            for layer in pathway.getLayers() :  # each layer that this pathway/fc has.
                funcList_AllFmActs_Preds.append( layer.fmsActivations([0, 9999]) )
                
        funcList_AllFmActs_Preds.append(self.finalTargetLayer.predictionProbabilities())
        ...
        for subPath_i in xrange(self.numSubsPaths) : # if there are subsampled paths...
            ...
            givensSet.update({ xSub: sharedInpXSubTest[index * self.batchSizeTesting: (index + 1) * \
                                                                        self.batchSizeTesting] })
    ...
    def _getInputTensorsXToCnn(self):
        if not self.inputTensorsXToCnnInitialized :
            # Symbolic variables, which stand for the input to the CNN. 
            #Will be loaded by the compiled trainining/val/test function. 
            #Can also be pre-set by an existing tensor if required in future extensions.
            tensor5 = T.TensorType(dtype='float32', broadcastable=(False, False, False, False, False))
            # Actually, for these 3, a single tensor5() could be used, as long as I reshape it separately for each afterwards. 
            #The actual value is loaded by the compiled functions.
            self.inputTensorNormTrain = tensor5()  
    ...
    def make_cnn_model( self,
                        ...
                        kernelDimensions,
                        #=== Subsampled Pathway ===
                        # THESE NEXT TWO, ALONG WITH THE ONES FOR FC, COULD BE PUT IN ONE STRUCTURE WITH NORMAL, 
                        #EG LIKE kerns = [ [kernsNorm], [kernsSub], [kernsFc]]
                        # Used to control if secondary pathways: [] if no secondary pathways. Now its the "factors"
                        nkernsSubsampled, 
                        kernelDimensionsSubsampled,
                        # Controls how many pathways: [] if no secondary pathways. Else, List of lists. 
                        #One sublist per secondary pathway. Each sublist has 3 ints, the rcz subsampling factors.
                        ...
                        #=== Others ===
                        # Dropout
                        # list of sublists, one for each pathway. Each either empty or full with the dropout
                        # rates of all the layers in the path.
                        dropoutRatesForAllPathways,  
                        # Initialization
        ...
        """
        maxPoolingParamsStructure: The padding of the function further below adds zeros. 
        Zeros are not good, especially if I use PreLus. So I made mine, that pads by mirroring.
        Be careful that, without this, if I have ds=2, str=1 and ignoreBorder=False, 
        it still reduces the dimension of the image by 1. That's why I need this. To keep the dimensions stable.
        ...
        # fcLayersFMs???
        self.kernelDimsFirstFcLayer = kernelDimsFirstFcLayer
        ...
        #This recField CNN should in future be calculated with all non-secondary pathways, ie normal+fc. 
        #Use another variable for pathway.recField.
        self.recFieldCnn = \
            calcReceptiveFieldDims(kernelDimensions)
#         print('debug-------------------------------------------------------------')
#         print('recFieldCnn',self.recFieldCnn)
        #==============================
        ...
        listInputTensorPerSubsTrain, listInputTensorPerSubsVal, \
                                    listInputTensorPerSubsTest) = self._getInputTensorsXToCnn()
        
        #=======================Make the NORMAL PATHWAY of the CNN=======================
        thisPathway = NormalPathway()
        self.pathways.append(thisPathway)
        pwType = thisPathway.pType()
        ...
        pwNKerns = nkerns
        pwKernelDims = kernelDimensions
        
        thisPathwayNumOfLayers = len(pwNKerns)
        thisPathwayUseBnPerLayer = [rollingAverageForBNOverThatManyBatches > 0] * thisPathwayNumOfLayers
        thisPathwayUseBnPerLayer[0] = applyBnToInputOfPathways[pwType] if \
            rollingAverageForBNOverThatManyBatches > 0 else False  # For the 1st layer, ask specific flag.
        
        thisPathwayActivFuncPerLayer = [actFuncToUseRelu0orPrelu1] * thisPathwayNumOfLayers
        # To not apply activation on raw input. -1 is linear activation.
        thisPathwayActivFuncPerLayer[0] = -1 if pwType <> pt.FC else actFuncToUseRelu0orPrelu1  
        
        thisPathway.makeLayersAndReturnDimsOfOutputFM(myLogger,
                                                                         inputToPathwayTrain,
                                                                         inputToPathwayVal,
                                                                         inputToPathwayTest,
                                                            ...
                                                            dropoutRatesForAllPathways[pwType],
                                                                         
                                                            maxPoolingParamsStructure[pwType],
                                                                         
                                                            indicesOfLowerRankLayersPerPathway[pwType],
                                                            ranksOfLowerRankLayersForEachPathway[pwType],
                                                            indicesOfLayersToConnectResidualsInOutput[pwType]
                                                                         )
        # Skip connections to end of pathway.
        thisPathway.makeMultiscaleConnectionsForLayerType(\
                                        convLayersToConnectToFirstFcForMultiscaleFromAllLayerTypes[pwType])
        
        [dimsOfOutputFrom1stPathwayTrain, dimsOfOutputFrom1stPathwayVal, dimsOfOutputFrom1stPathwayTest] = \
        ...
        for subPath_i in xrange(self.numSubsPaths) :
            ...
            pwNKerns = nkernsSubsampled[subPath_i]
            pwKernelDims = kernelDimensionsSubsampled
            ...
            thisPathwayActivFuncPerLayer = [actFuncToUseRelu0orPrelu1] * thisPathwayNumOfLayers
            thisPathwayActivFuncPerLayer[0] = -1 if pwType <> pt.FC else \
                 actFuncToUseRelu0orPrelu1  # To not apply activation on raw input. -1 is linear activation.
            
            inputToPathwayShapeTrain = [self.batchSize, numberOfImageChannelsPath2] + \
                thisPathway.calcInputRczDims(pwKernelDims, dimsOfOutputFrom1stPathwayTrain);
            inputToPathwayShapeVal = [self.batchSizeValidation, numberOfImageChannelsPath2] + \
                thisPathway.calcInputRczDims(pwKernelDims, dimsOfOutputFrom1stPathwayVal)
            inputToPathwayShapeTest = [self.batchSizeTesting, numberOfImageChannelsPath2] + \
                thisPathway.calcInputRczDims(pwKernelDims, dimsOfOutputFrom1stPathwayTest)
            
#             print('debug --------------------------------------------------------------')
#             print('inputToPathwayShapeTrain',inputToPathwayShapeTrain,inputToPathwayShapeVal,inputToPathwayShapeTest)
#             print('dimsOfOutputFrom1stPathwayTrain',dimsOfOutputFrom1stPathwayTrain,dimsOfOutputFrom1stPathwayVal,dimsOfOutputFrom1stPathwayTest)
            thisPathway.makeLayersAndReturnDimsOfOutputFM(myLogger,
                                                                     inputToPathwayTrain,
                                                                     inputToPathwayVal,
                                                                     inputToPathwayTest,
        ...
        inputToFirstFcLayerTrain = None; inputToFirstFcLayerVal = None; 
        inputToFirstFcLayerTest = None; numberOfFmsOfInputToFirstFcLayer = 0
        for path_i in xrange(len(self.pathways)) :
            [outputNormResOfPathTrain, outputNormResOfPathVal, outputNormResOfPathTest] = \
                                            self.pathways[path_i].getOutputAtNormalRes()
            [dimsOfOutputNormResOfPathTrain, dimsOfOutputNormResOfPathVal, dimsOfOutputNormResOfPathTest] = \
                                                    self.pathways[path_i].getShapeOfOutputAtNormalRes()
            
            inputToFirstFcLayerTrain =  T.concatenate([inputToFirstFcLayerTrain, outputNormResOfPathTrain], axis=1) if \
                                                                        path_i <> 0 else outputNormResOfPathTrain
            inputToFirstFcLayerVal = T.concatenate([inputToFirstFcLayerVal, outputNormResOfPathVal], axis=1) if \
                                                                        path_i <> 0 else outputNormResOfPathVal
            inputToFirstFcLayerTest = T.concatenate([inputToFirstFcLayerTest, outputNormResOfPathTest], axis=1) if \
                                                                        path_i <> 0 else outputNormResOfPathTest
        ...
        # However I can now define it larger (eg 3x3x3), 
        #in case it helps combining the multiresolution features better/smoother.
        # The convolution is seamless, ie same shape output/input, by mirror padding the input.
        firstFcLayerAfterConcatKernelShape = self.kernelDimsFirstFcLayer
        voxelsToPadPerDim = [ kernelDim - 1 for kernelDim in firstFcLayerAfterConcatKernelShape ]
        myLogger.print3("DEBUG: Shape of the kernel of the first FC layer is : " + \
                        str(firstFcLayerAfterConcatKernelShape))
        myLogger.print3("DEBUG: Input to the FC Pathway will be padded by that many voxels per dimension: " + \
                        str(voxelsToPadPerDim))
        inputToPathwayTrain = padImgWithMirror(inputToFirstFcLayerTrain, voxelsToPadPerDim)
        inputToPathwayVal = padImgWithMirror(inputToFirstFcLayerVal, voxelsToPadPerDim)
        inputToPathwayTest = padImgWithMirror(inputToFirstFcLayerTest, voxelsToPadPerDim)
        inputToPathwayShapeTrain = [self.batchSize, numberOfFmsOfInputToFirstFcLayer] + \
                                            dimsOfOutputFrom1stPathwayTrain[2:5]
        inputToPathwayShapeVal = [self.batchSizeValidation, numberOfFmsOfInputToFirstFcLayer] + \
                                            dimsOfOutputFrom1stPathwayVal[2:5]
        inputToPathwayShapeTest = [self.batchSizeTesting, numberOfFmsOfInputToFirstFcLayer] + \
        ...
        pwNKerns = fcLayersFMs + [self.numberOfOutputClasses]
        pwKernelDims = [firstFcLayerAfterConcatKernelShape] + \
                                        [[1, 1, 1]] * (len(pwNKerns) - 1)
