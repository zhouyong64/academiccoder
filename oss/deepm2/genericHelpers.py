...
def strFlXfNA(floatOrNotAppl, numDec, notApplicPattern) :
    stringThatSaysHowManyDecimals = "{:." + str(numDec) + "f}"
    return stringThatSaysHowManyDecimals.format(floatOrNotAppl) if floatOrNotAppl <> notApplicPattern \
                                                                else notApplicPattern
...
def strListFlXfNA(listWithFloatsOrNotAppl, numDec, notApplicPattern) : # If you just do a normal list, \
    #the internal string-floats are printed with quotes ' '
    stringThatSaysHowManyDecimals = "%." + str(numDec) + "f"
    strList = "[ "
    for element in listWithFloatsOrNotAppl :
        strList += stringThatSaysHowManyDecimals % element + " " if element <> notApplicPattern \
                                                                else notApplicPattern + " "
    strList += "]"
    return strList
def strListFl4fNA(listWithFloatsOrNotAppl, notApplicPattern) : # If you just print a normal list, \
    #the internal string-floats are printed with quotes ' '. To avoit it we use this.
    return strListFlXfNA(listWithFloatsOrNotAppl, 4, notApplicPattern)
def strListFl5fNA(listWithFloatsOrNotAppl, notApplicPattern) : # If you just print a normal list, \
    #the internal string-floats are printed with quotes ' '. To avoit it we use this.
...
def getMeanPerColOf2dListExclNA(list2d, notApplicPattern) :
    ...
    # Return a list, which has 1 element per column, which is the average of the column, 
    # excluding any "NotApplicable" elements.
...
def calcDiceCoef(predictedBinaryLabels, groundTruthBinaryLabels) :
    ...
    diceCoeff = (2.0 * numberOfTruePositives) / (np.sum(predictedBinaryLabels) + numberOfGtPositives) if \
                                                                            numberOfGtPositives<>0 else -1 
    return diceCoeff
...
def get_random_subject_indices_to_load_on_GPU(total_number_of_subjects, 
    ...
    if max_subjects_on_gpu_for_subepoch>=total_number_of_subjects:
        ...
        if get_max_subjects_for_gpu_even_if_total_less : #This is if I want to have a certain amount on GPU, \
            # even if total subjects are less.
            while (len(random_order_chosen_subjects)<max_subjects_on_gpu_for_subepoch):
                random.shuffle(subjects_indices)
                number_of_extra_subjects_to_get_to_fill_gpu = \
                    min(max_subjects_on_gpu_for_subepoch - len(random_order_chosen_subjects), total_number_of_subjects)
...
def getSuggestedStdForSubsampledImage(subsampleFactor) :
    ...
    #if subsampledFactor == 1 for a certain axis (eg z axis), it means I am actually doing 2D processing. 
    # In this case, use std=0 on this axis, and I dont smooth at all. I do clean slice-by-slice.
...
def saveImageToANewNiiWithHeaderFromOtherGivenExactFilePaths(predLabelImg,
    ...
    newLabelImg = nib.Nifti1Image(predLabelImg, affine_trans_to_ras) 
    newLabelImg.set_data_dtype(npDtype)
    
    dimensionsOfTheGivenArrayImageToSave = len(predLabelImg.shape)
    newZooms = list(hdr_for_orig_image.get_zooms()[:dimensionsOfTheGivenArrayImageToSave])
    #Eg if original image was 3D, but I need to save a multichannel image.
    if len(newZooms) < dimensionsOfTheGivenArrayImageToSave : 
...
def savePredictedImageToANewNiiWithHeaderFromOther(predLabelImg,
                                                   listOfNamesToGiveToPredictions,
                                                   fpathsToEachChannelOfEachPat,
                                                   case_i, #the index (in the list of filepathnames) of \
                                                   #the current image segmented.
                                                   suffixToAdd = "",
                                                   npDtype = np.dtype(np.float32),
                                                   myLogger=None) :
    ...
    ali = predLabelImg
#     print('pred type',type(predLabelImg),predLabelImg.shape)
    print(ali.mean(),ali.min(),ali.max(),ali.shape)

    fullFilenameOfOriginalImageToCopyHeader = fpathsToEachChannelOfEachPat[case_i][0]
    ...
    saveImageToANewNiiWithHeaderFromOtherGivenExactFilePaths(   predLabelImg,
                                                                fullFilenameToSaveWith,
                                                                fullFilenameOfOriginalImageToCopyHeader,
                                                                npDtype,
                                                                myLogger)
...
def saveFmActivationImageToANewNiiWithHeaderFromOther(  fmImageCreatedByVisualisation,
                                                        ...
                                                        myLogger=None) : 
    #the index (in the list of filepathnames) of the current image segmented :
    #give as arguments the list of the patient filepaths and the index of the currently segmented image, so that ...
    #... I can get the header, affine RAS trans etc from it and copy it for the new image.
    
    stringToPrint = "Saving the new Fm-activation image for the subject #"+str(image_i)+", pathway:" + \
                                                        str(index_of_typeOfPathway_to_visualize)\
    ...
    fullFilenameOfOriginalImageToCopyHeader = fpathsToEachChannelOfEachPat[image_i][0]
    fullFilenameToSaveWith = "PLACEHOLDER"
    if listOfNamesToGiveToPredictions[image_i].endswith(".nii.gz") :
        fullFilenameToSaveWith = listOfNamesToGiveToPredictions[image_i][:-7] + "_pathway" + \
        str(index_of_typeOfPathway_to_visualize)\
        + "_layer" + str(index_of_layer_in_pathway_to_visualize) + "_fm" + \
        str(index_of_FM_in_pathway_to_visualize) + ".nii.gz"
    elif listOfNamesToGiveToPredictions[image_i].endswith(".nii") :
        fullFilenameToSaveWith = listOfNamesToGiveToPredictions[image_i][:-4] + "_pathway" + \
            str(index_of_typeOfPathway_to_visualize)\
        + "_layer" + str(index_of_layer_in_pathway_to_visualize) + "_fm" + str(index_of_FM_in_pathway_to_visualize) + \
                                                                                                        ".nii.gz"
    else :
        fullFilenameToSaveWith = listOfNamesToGiveToPredictions[image_i] + "_pathway" + \
                str(index_of_typeOfPathway_to_visualize)\
        + "_layer" + str(index_of_layer_in_pathway_to_visualize) + "_fm" + \
                str(index_of_FM_in_pathway_to_visualize) + ".nii.gz"
...
def saveMDImgWithAllVisualisedFmsToANewNiiWithHeaderFromOther(multidimImageWithAllVisualisedFms,
    ...
    fullFilenameOfOriginalImageToCopyHeader = fpathsToEachChannelOfEachPat[image_i][0]
    fullFilenameToSaveWith = "PLACEHOLDER"
    if namesToGiveToFmVisualisationsIfSaving[image_i].endswith(".nii.gz") :
        fullFilenameToSaveWith = namesToGiveToFmVisualisationsIfSaving[image_i][:-7] + "_allFmsMultiDim.nii.gz"
    elif namesToGiveToFmVisualisationsIfSaving[image_i].endswith(".nii") :
        fullFilenameToSaveWith = namesToGiveToFmVisualisationsIfSaving[image_i][:-4] + "_allFmsMultiDim.nii.gz"
    else :
        fullFilenameToSaveWith = namesToGiveToFmVisualisationsIfSaving[image_i] + "_allFmsMultiDim.nii.gz"
