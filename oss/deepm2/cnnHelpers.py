...
class CnnWrapperForSampling(object):
    # Only for the parallel process used during training. So that it won't re-load theano etc. 
    ...
    def __init__(self, cnn3dInstance) :
        ...
        self.batchSizeTrainValTest = [ cnn3dInstance.batchSize, cnn3dInstance.batchSizeValidation, \
                                      cnn3dInstance.batchSizeTesting ]
...
def calculateSubsampledImagePartDimensionsFromImagePartSizePatchSizeAndSubsampleFactor(imagePartDimensions, \
                                                                            patchDimensions, subsampleFactor) :
    """
    This function gives you how big your subsampled-image-part should be, 
    so that it corresponds to the correct number of central-voxels in the normal-part. Currently, 
    it's coupled with the patch-size of the normal-scale. 
    I.e. the subsampled-patch HAS TO BE THE SAME SIZE as the normal-scale, 
    and corresponds to subFactor*patchsize in context.
    When the central voxels are not a multiple of the subFactor, you get ceil(), 
    so +1 sub-patch. When the CNN repeats the pattern, 
    it is giving dimension higher than the central-voxels of the normal-part, 
    but then they are sliced-down to the correct number (in the cnn_make_model function, right after the repeat).        
    This function works like this because of getImagePartFromSubsampledImageForTraining(), 
    which gets a subsampled-image-part by going 1 normal-patch back from the top-left voxel of a normal-scale-part, 
    and then 3 ahead. If I change it to start from the top-left-CENTRAL-voxel back and front, 
    I will be able to decouple the normal-patch size and the subsampled-patch-size. 
    """
    #if patch is 17x17, a 17x17 subPart is cool for 3 voxels with a subsampleFactor. 
    ...
    for rcz_i in xrange(len(imagePartDimensions)) :
        ...
        centralVoxelsInThisDimensionForSubsampledPart = \
                            int(ceil(centralVoxelsInThisDimension*1.0/subsampleFactor[rcz_i]))
        sizeOfSubsampledImagePartInThisDimension = patchDimensions[rcz_i] + \
                            centralVoxelsInThisDimensionForSubsampledPart - 1
        subsampledImagePartDimensions.append(sizeOfSubsampledImagePartInThisDimension)
    return subsampledImagePartDimensions

#Joe: renamed from 'calculateReceptiveFieldDimensionsFromKernelsDimListPerLayerForFullyConvCnnWithStrides1'
def calcReceptiveFieldDims(kernDimPerLayerList) :
...
def checkReceptiveFieldFineInComparisonToSegmentSize(receptiveFieldDim, segmentDim) :
    ...
    if numberOfRFDim <> numberOfSegmDim :
        print "ERROR: [in function checkReceptiveFieldFineInComparisonToSegmentSize()] : \
                Receptive field and image segment have different number of dimensions! (should be 3 for both! Exiting!)"
        exit(1)
    for dim_i in xrange(numberOfRFDim) :
        if receptiveFieldDim[dim_i] > segmentDim[dim_i] :
            print "ERROR: [in function checkReceptiveFieldFineInComparisonToSegmentSize()] : \
            The segment-size (input) should be at least as big as the receptive field of the model! \
            This was not found to hold! Dimensions of Receptive Field:", receptiveFieldDim, \
            ". Dimensions of Segment: ", segmentDim
            return False
    return True

def checkKernDimPerLayerCorrect3dAndNumLayers(kernDimensionsPerLayer, numOfLayers) :
    #kernDimensionsPerLayer : a list with sublists. One sublist per layer. Each sublist should have 3 integers, 
    #specifying the dimensions of the kernel at the corresponding layer of the pathway. 
    #eg: kernDimensionsPerLayer = [ [3,3,3], [3,3,3], [5,5,5] ] 
