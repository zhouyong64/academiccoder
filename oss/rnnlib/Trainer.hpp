...
struct Trainer: public DataExporter
{
	...
	//functions
	Trainer(ostream& o, Mdrnn* n, ConfigFile& conf, const string& name = "trainer"):
		DataExporter(name),
		out(o),
		net(n),
		optimiser(0),
		...
		mdlSymmetricSampling(mdl ? config.get<bool>("mdlSymmetricSampling", false) : false),
		mdlPriorMean(0),
		mdlPriorStdDev(0),
		mdlPriorVariance(0),
		mdlOptimiser(0)
	{
		...
		if(optType == "rprop")
		{
			optimiser = new Rprop(optName, out, wts, derivs);
		}
		else
		{
			optimiser = new SteepestDescent(optName, out, wts, derivs, learnRate, momentum);
		}
		if(mdl)
		{
			...
			if(mdlOptType == "rprop")
			{
				mdlOptimiser = new Rprop(mdlOptName, out, mdlStdDevs, mdlStdDevDerivs);
			}
			else
			{
            ...
            WeightContainer::instance().save_by_conns(weightCosts, "_mdl_weight_costs");
		}
		SAVE(epoch);
	}
	...
	real_t mdl_evaluate()
	{
		...
		FOR(i, wts.size())
		{
			weightNats += KL_normal(mdl_mean(i), mdl_variance(i), mdlPriorMean, mdlPriorVariance);;
		}
	...
	void mdl_differentiate(real_t scaleFactor = 1)
	{
        ...
        LOOP(int i, indices(derivs))
        {
            ...
            mdlStdDevDerivs[i] += scaleFactor * ((stdDev/mdlPriorVariance) - (1/stdDev));
        }
	}
	...
	{
		return KL_normal(mdl_mean(i), mdl_variance(i), mdlPriorMean, mdlPriorVariance);
	}
	...
	void calculate_compression_errors(DatasetErrors& errors)
	{
		if (mdl && in(errors.errors, criteria.front()))
		{
			...
			if ((mdl) && ((&errors)==(&trainErrors)))
			{
				...
				if (errors.normFactors.find("bitsPerSymbol") != errors.normFactors.end())
				{
					errors.errors["totalBitsPerSymbol"] = totalBits / errors.normFactors["bitsPerSymbol"];
				}
			}
			...
			for (const DataSequence* seq = trainFiles.start(); seq; seq = trainFiles.next_sequence())
			{
				...
				if (verbose)
				{
					...
					if(mdl)
					{
						prt_line(out);
						mdl_print_stats();
						out << "bitsPerWeight = " << nats_to_bits(mdl_evaluate()) / wts.size() << endl;
						prt_line(out);
					}
					out << endl;
				}
	...
	bool print_distortions()
	{
		...
		if (weightDistortion)
		{
			out << "adding  noise to weights every sequence, std dev " << weightDistortion << endl << endl; 
			distortions = true;
		}
		if (mdl)
		{
