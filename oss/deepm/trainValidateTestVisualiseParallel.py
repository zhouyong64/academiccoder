...
def padCnnInputs(array1, cnnReceptiveField, imagePartDimensions) : #Works for 2D as well I think.
    ...
    paddingFurtherToTheRightNeededForSegment = np.maximum(0, np.asarray(imagePartDimensions,dtype="int16")-\
                                                (array1Dimensions+paddingAtLeftPerAxis+paddingAtRightPerAxis))
...
def unpadCnnOutputs(array1, tupleOfPaddingPerAxesLeftRight) :
    ...
    unpaddedArray1 = array1[tupleOfPaddingPerAxesLeftRight[0][0]:, tupleOfPaddingPerAxesLeftRight[1][0]:, \
    ...
    unpaddedArray1 = unpaddedArray1[:-tupleOfPaddingPerAxesLeftRight[0][1],:,:] if \
        tupleOfPaddingPerAxesLeftRight[0][1] > 0 else unpaddedArray1 
    unpaddedArray1 = unpaddedArray1[:,:-tupleOfPaddingPerAxesLeftRight[1][1],:] if \
        tupleOfPaddingPerAxesLeftRight[1][1] > 0 else unpaddedArray1
    unpaddedArray1 = unpaddedArray1[:,:,:-tupleOfPaddingPerAxesLeftRight[2][1]] if \
        tupleOfPaddingPerAxesLeftRight[2][1] > 0 else unpaddedArray1
    return unpaddedArray1

def reflectImageArrayIfNeeded(reflectFlags, imageArray) :
    stepsForReflectionPerDimension = [-1 if reflectFlags[0] else 1, -1 if reflectFlags[1] else 1, -1 if\
...
def actual_load_patient_images_from_filepath_and_return_nparrays(myLogger,
    ...
    if index_of_wanted_image >= len(listOfFilepathsToEachChannelOfEachPatient) :
        ...
        myLogger.print3("------- The argument 'index_of_wanted_image' given is greater than the filenames \
            given for the .nii folders! Exiting.")
        exit(1)
    
    myLogger.print3("Loading subject with 1st channel at: "+\
                    str(listOfFilepathsToEachChannelOfEachPatient[index_of_wanted_image][0]))
    ...
    if providedRoiMaskBool :
        ...
        #it is not a float64
        img_proxy.uncache()
        [roiMask, tupleOfPaddingPerAxesLeftRight] = padCnnInputs(roiMask, cnnReceptiveField, dimsOfPrimeSegmentRcz) if \
            padInputImagesBool else [roiMask, tupleOfPaddingPerAxesLeftRight]
    else :
    ...
    howMuchToAddAndMultiplyForNormalizationAugmentationForEachChannel = \
    ...
    for channel_i in xrange(numberOfNormalScaleChannels):
        ...
        if fullFilenamePathOfChannel <> "-" : #normal case, filepath was given.
            ...
            [channelData, tupleOfPaddingPerAxesLeftRight] = padCnnInputs(channelData, \
                        cnnReceptiveField, dimsOfPrimeSegmentRcz) if padInputImagesBool else\
                                                 [channelData, tupleOfPaddingPerAxesLeftRight]
            ...
            if not isinstance(allChannelsOfPatientInNpArray, (np.ndarray)) :
                ...
                allChannelsOfPatientInNpArray = np.zeros( (numberOfNormalScaleChannels, niiDimensions[0], \
                                                           niiDimensions[1], niiDimensions[2]))
        ...
        if training0orValidation1orTest2 == 0 and \
            normAugmNone0OnImages1OrSegments2AlreadyNormalized1SubtrUpToPropOfStdAndDivideWithUpToPerc[0] == 1 : #[0] \
            #should be ==0 for no normAug, eg in the case of validation
            stdOfChannel = 1.
            if normAugmNone0OnImages1OrSegments2AlreadyNormalized1SubtrUpToPropOfStdAndDivideWithUpToPerc[1] == 0 : #[0]==0 \
            ...
            #Draw from gaussian
            howMuchToAddAndMultiplyForNormalizationAugmentationForEachChannel[channel_i][0] = \
                random.normalvariate(\
                 normAugmNone0OnImages1OrSegments2AlreadyNormalized1SubtrUpToPropOfStdAndDivideWithUpToPerc[1][0], \
                    normAugmNone0OnImages1OrSegments2AlreadyNormalized1SubtrUpToPropOfStdAndDivideWithUpToPerc[1][1]) * \
                        stdOfChannel
            howMuchToAddAndMultiplyForNormalizationAugmentationForEachChannel[channel_i][1] = \
                random.normalvariate(\
                    normAugmNone0OnImages1OrSegments2AlreadyNormalized1SubtrUpToPropOfStdAndDivideWithUpToPerc[2][0], \
                        normAugmNone0OnImages1OrSegments2AlreadyNormalized1SubtrUpToPropOfStdAndDivideWithUpToPerc[2][1])
    ...
    if providedGtLabelsBool : #For training (exact target labels) or validation on samples labels.
        ...
        [imageGtLabels, tupleOfPaddingPerAxesLeftRight] = padCnnInputs(imageGtLabels, cnnReceptiveField, \
                                            dimsOfPrimeSegmentRcz) if padInputImagesBool else \
                                                [imageGtLabels, tupleOfPaddingPerAxesLeftRight]
    else : 
    ...
    if training0orValidation1orTest2 <> 2 and providedWeightMapsToSampleForEachCategory==True : # in testing these \
        ...
        arrayWithWeightMapsWhereToSampleForEachCategory = np.zeros( [numberOfSamplingCategories] + \
        ...
        for cat_i in xrange( numberOfSamplingCategories ) :
            filepathsToTheWeightMapsOfAllPatientsForThisCategory = \
                    forEachSamplingCategory_aListOfFilepathsToWeightMapsOfEachPatient[cat_i]
            filepathToTheWeightMapOfThisPatientForThisCategory = \
                    filepathsToTheWeightMapsOfAllPatientsForThisCategory[index_of_wanted_image]
            ...
            img_proxy.uncache()
            [weightedMapForThisCatData, tupleOfPaddingPerAxesLeftRight] = padCnnInputs(weightedMapForThisCatData, \
                                cnnReceptiveField, dimsOfPrimeSegmentRcz) if padInputImagesBool else \
                                    [weightedMapForThisCatData, tupleOfPaddingPerAxesLeftRight]
    ...
    if not usingSubsampledPathways :
        ...
        allSubsampledChannelsOfPatientInNpArray = allChannelsOfPatientInNpArray 
    else :
        numberOfSubsampledScaleChannels = len(listOfFilepathsToEachSubsampledChannelOfEachPatient[0])
        allSubsampledChannelsOfPatientInNpArray = np.zeros( (numberOfSubsampledScaleChannels, niiDimensions[0], \
                                                             niiDimensions[1], niiDimensions[2]))
        for channel_i in xrange(numberOfSubsampledScaleChannels):
            fullFilenamePathOfChannel = \
                listOfFilepathsToEachSubsampledChannelOfEachPatient[index_of_wanted_image][channel_i]
            img_proxy = nib.load(fullFilenamePathOfChannel)
            channelData = img_proxy.get_data()
            if len(channelData.shape) > 3 :
                channelData = channelData[:,:,:,0]
            ...
            if len(channelData.shape) <= 3 :
                ...
                allSubsampledChannelsOfPatientInNpArray[channel_i] = channelData[:,:,:,0] \
            ...
            #-------For Data Augmentation when it comes to normalisation values--------------
            if training0orValidation1orTest2 == 0 and \
                    normAugmNone0OnImages1OrSegments2AlreadyNormalized1SubtrUpToPropOfStdAndDivideWithUpToPerc[0] == 1 :
                ...
                valueToAddToEachVoxel = howMuchToAddAndMultiplyForNormalizationAugmentationForEachChannel[channel_i][0]
                valueToMultiplyEachVoxel = howMuchToAddAndMultiplyForNormalizationAugmentationForEachChannel[channel_i][1]
                allSubsampledChannelsOfPatientInNpArray[channel_i] = (allSubsampledChannelsOfPatientInNpArray[channel_i] \
                                                                      + valueToAddToEachVoxel)*valueToMultiplyEachVoxel
                
    return [allChannelsOfPatientInNpArray, imageGtLabels, roiMask, arrayWithWeightMapsWhereToSampleForEachCategory, \
                                        allSubsampledChannelsOfPatientInNpArray, tupleOfPaddingPerAxesLeftRight]
...
def extractDataOfSegmentsUsingSampledSliceCoords(cnn3dInstance,
    ...
    channsForSegmentsPerPathToReturn = [ [] for i in xrange(cnn3dInstance.getNumPathwaysThatRequireInput()) ] 
    ...
    for segment_i in xrange(numberOfSegmentsToExtract) :
        ...
        for pathway_i in xrange(len(cnn3dInstance.pathways)) : # Except Normal 1st, cause that was done already.
            if cnn3dInstance.pathways[pathway_i].pType() == pt.FC or \
                cnn3dInstance.pathways[pathway_i].pType() == pt.NORM:
                continue
...
def extractDataOfASegmentFromImagesUsingSampledSliceCoords(
    ...
    for pathway in cnn3d.pathways[:1] : #Hack. The rest of this loop can work for the whole .pathways...
        ...
        if training0orValidation1 == 0 and \
                normAugmNone0OnImages1OrSegments2AlreadyNormalized1SubtrUpToPropOfStdAndDivideWithUpToPerc[0] == 2 : 
            if howMuchToAddForEachChannel == None or howMuchToMultiplyForEachChannel == None :
                ...
                muOfGaussToMultiply = \
                    normAugmNone0OnImages1OrSegments2AlreadyNormalized1SubtrUpToPropOfStdAndDivideWithUpToPerc[3][0]
                stdOfGaussToMultiply = \
                    normAugmNone0OnImages1OrSegments2AlreadyNormalized1SubtrUpToPropOfStdAndDivideWithUpToPerc[3][1]
...
def getTheArraysOfImageChannelsAndLesionsToLoadToGpuForSubepoch(myLogger,
    ...
    for index_for_vector_with_images_on_gpu in xrange(0, numOfSubjectsLoadingThisSubepochForSampling) :
        ...
        gtLabelsImage,
        roiMask,
        arrayWithWeightMapsWhereToSampleForEachCategory, #can be returned "placeholderNothing" if it's \
        ...
        if training0orValidation1 == 0 and \
                normAugmNone0OnImages1OrSegments2AlreadyNormalized1SubtrUpToPropOfStdAndDivideWithUpToPerc[0] == 2 and\
            normAugmNone0OnImages1OrSegments2AlreadyNormalized1SubtrUpToPropOfStdAndDivideWithUpToPerc[1] == 0:\
...
def doTrainOrValidationOnBatchesAndReturnMeanAccuraciesOfSubepoch(myLogger,
                                                                train0orValidation1,
                                                                number_of_batches, #This is the integer division of \
    ...
    for batch_i in xrange(number_of_batches):
        ...
        if  batch_i%printProgressStep == 0 :
            myLogger.print3( trainedOrValidatedString + " on "+str(batch_i)+"/"+str(number_of_batches)+\
                             " of the batches for this subepoch...")
        if train0orValidation1==0 : #training
            listWithCostMeanErrorAndRpRnTpTnForEachClassFromTraining = cnn3dInstance.cnnTrainModel(batch_i, \
                                                                vectorWithWeightsOfTheClassesForCostFunctionOfTraining)
            #I should put this inside the 3dCNN.
            cnn3dInstance.updateTheMatricesOfTheLayersWithTheLastMusAndVarsForTheMovingAverageOfTheBatchNormInference() 
    ...
    meanCostOfSubepoch = accuracyMonitorForEpoch.NA_PATTERN if (train0orValidation1 == 1) else \
                                                sum(costsOfBatches) / float(number_of_batches)
...
def do_training(myLogger,
    ...
    while cnn3dInstance.numberOfEpochsTrained < n_epochs :
        ...
        trainingAccuracyMonitorForEpoch = AccuracyOfEpochMonitorSegmentation(myLogger, 0, \
                            cnn3dInstance.numberOfEpochsTrained, cnn3dInstance.numberOfOutputClasses, number_of_subepochs)
        validationAccuracyMonitorForEpoch = None if not performValidationOnSamplesDuringTrainingProcessBool else \
                                        AccuracyOfEpochMonitorSegmentation(myLogger, 1, \
                                                cnn3dInstance.numberOfEpochsTrained, \
                                                cnn3dInstance.numberOfOutputClasses, number_of_subepochs ) 
...
#             myLogger.print3("**************************************************************************************************")
            ...
            if performValidationOnSamplesDuringTrainingProcessBool :
                ...
                numberOfBatchesValidation = len(channsOfSegmentsForSubepPerPathwayVal[0]) / \
                    cnn3dInstance.batchSizeValidation #Computed with number of extracted samples, \
                ...
                for index in xrange(len(channsOfSegmentsForSubepPerPathwayVal[1:])) :
                    cnn3dInstance.sharedInpXPerSubsListVal[index].set_value(channsOfSegmentsForSubepPerPathwayVal[1+index], \
                                                                            borrow=borrowFlag)
            ...
            if cnn3dInstance.numberOfEpochsTrained < numberOfEpochsToWeightTheClassesInTheCostFunction :
                ...
                y1 = (1./(actualNumOfPatchesPerClassInTheSubepoch_notParts+TINY_FLOAT)) * \
                    (numOfPatchesInTheSubepoch_notParts*1.0/cnn3dInstance.numberOfOutputClasses)
                y2 = 1.
                ...
                vectorWithWeightsOfTheClassesForCostFunctionOfTraining = \
                    np.ones(cnn3dInstance.numberOfOutputClasses, dtype='float32')
            ...
            numberOfBatchesTraining = len(channsOfSegmentsForSubepPerPathwayTrain[0]) / cnn3dInstance.batchSize 
            
            # Primary pathway
            cnn3dInstance.sharedInpXTrain.set_value(channsOfSegmentsForSubepPerPathwayTrain[0], borrow=borrowFlag) 
            for index in xrange(len(channsOfSegmentsForSubepPerPathwayTrain[1:])) :
                cnn3dInstance.sharedInpXPerSubsListTrain[index].set_value(channsOfSegmentsForSubepPerPathwayTrain[1+index], \
                                                                          borrow=borrowFlag)
...
#         myLogger.print3("~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~" )
        ...
        if (lowerLrByStable0orAuto1orPredefined2orExponential3Schedule == 0) and \
            (numEpochsToWaitBeforeLowerLR > 0) and (cnn3dInstance.numberOfEpochsTrained % numEpochsToWaitBeforeLowerLR)==0 :
            # STABLE LR SCHEDULE"
            myLogger.print3("DEBUG: Going to lower Learning Rate because of STABLE schedule! \
                The CNN has now been trained for: " + str(cnn3dInstance.numberOfEpochsTrained) + \
            ...
            if not performValidationOnSamplesDuringTrainingProcessBool : 
                ...
                exit(1)
            if (cnn3dInstance.numberOfEpochsTrained >= cnn3dInstance.topMeanValidationAccuracyAchievedInEpoch[1] + \
                    numEpochsToWaitBeforeLowerLR) and \
                        (cnn3dInstance.numberOfEpochsTrained >= cnn3dInstance.lastEpochAtTheEndOfWhichLrWasLowered + \
                         numEpochsToWaitBeforeLowerLR) :
                myLogger.print3("DEBUG: Going to lower Learning Rate because of AUTO schedule! The CNN has now been \
                    trained for: " + str(cnn3dInstance.numberOfEpochsTrained) + \
                    " epochs. Epoch with last highest achieved validation accuracy: " + \
                    str(cnn3dInstance.topMeanValidationAccuracyAchievedInEpoch[1]) + \
                    ", and epoch that Learning Rate was last lowered: " + \
                    str(cnn3dInstance.lastEpochAtTheEndOfWhichLrWasLowered) + \
                ...
                (cnn3dInstance.numberOfEpochsTrained in lowerLrAtTheEndOfTheseEpochsPredefinedScheduleList) :
            #Predefined Schedule.
            ...
            cnn3dInstance.divide_learning_rate_of_a_cnn_by(divideLrBy, myLogger)
                ...
                cnn3dInstance.numberOfEpochsTrained >= exponentialScheduleForLrAndMom[0]) :
            myLogger.print3("DEBUG: Going to lower Learning Rate and Increase Momentum because of \
                ...
                str(cnn3dInstance.numberOfEpochsTrained) + " epochs.")
            ...
            gammaForExpSchedule = pow( ( cnn3dInstance.initialLearningRate*exponentialScheduleForLrAndMom[1] * 1.0) / \
                                       cnn3dInstance.initialLearningRate, 1.0 / (n_epochs-minEpochToLowerLr))
            newLearningRate = cnn3dInstance.initialLearningRate * pow(gammaForExpSchedule, \
                                                            cnn3dInstance.numberOfEpochsTrained-minEpochToLowerLr + 1.0)
            #Momentum increased linearly.
            newMomentum = ((cnn3dInstance.numberOfEpochsTrained - minEpochToLowerLr + 1) - \
                           (n_epochs-minEpochToLowerLr))*1.0 / (n_epochs - minEpochToLowerLr) * \
                           (exponentialScheduleForLrAndMom[2] - cnn3dInstance.initialMomentum) + \
        ...
        if performFullInferenceOnValidationImagesEveryFewEpochsBool and (cnn3dInstance.numberOfEpochsTrained <> 0) and \
             (cnn3dInstance.numberOfEpochsTrained % everyThatManyEpochsComputeDiceOnTheFullValidationImages == 0) :
            ...
            performInferenceForTestingOnWholeVolumes(myLogger,
                                    ...
                                    borrowFlag,
                                    listOfNamesToGiveToPredictionsIfSavingResults = "Placeholder" if not \
...
def performInferenceForTestingOnWholeVolumes(myLogger,
                            ...
                            indicesOfFmsToVisualisePerPathwayTypeAndPerLayer,#NOTE: saveIndividualFmImagesForVisualisation \
...
#     myLogger.print3("###########################################################################################################")
    myLogger.print3("############################# Starting full Segmentation of " + str(validationOrTestingString) + \
                    " subjects ##########################")
#     myLogger.print3("###########################################################################################################")
    ...
    diceCoeffs2 = [ [-1] * NUMBER_OF_CLASSES for i in xrange(total_number_of_images) ] 
    #predictedInsideBrainMask/ LesionsInsideBrainMAsk (for comparisons)
    diceCoeffs3 = [ [-1] * NUMBER_OF_CLASSES for i in xrange(total_number_of_images) ] 
                ...
                for layer_i in xrange(len(pathway.getLayers())) :
                    ...
                    if indicesOfFmsToVisualiseForCertainLayerOfCertainPathway<>[] :
                        ...
                        indicesOfFmsToVisualiseForCertainLayerOfCertainPathway[1] = \
                            min(indicesOfFmsToVisualiseForCertainLayerOfCertainPathway[1], numberOfFeatureMapsInThisLayer)
    ...
    for image_i in xrange(total_number_of_images) :
        ...
        # All 0s when no padding.
                                        ...
                                        dimsOfPrimeSegmentRcz = \
                                            cnn3dInstance.pathways[0].getShapeOfInput()[2][2:], # only used if padInputsBool
        ...
        if saveIndividualFmImagesForVisualisation or saveMultidimensionalImageWithAllFms:
            multidimensionalImageWithAllToBeVisualisedFmsArray =  \
                np.zeros([totalNumberOfFMsToProcess] + niiDimensions, dtype = "float32")
        ...
        for batch_i in xrange(number_of_batches) : #batch_size = how many image parts in one batch. Has to be the \
            ...
            cnn3dInstance.sharedInpXTest.set_value(np.asarray(channsOfSegmentsPerPath[0], dtype='float32'), \
                                                   borrow=borrowFlag)
            for index in xrange(len(channsOfSegmentsPerPath[1:])) :
                cnn3dInstance.sharedInpXPerSubsListTest[index].set_value(\
                                    np.asarray(channsOfSegmentsPerPath[1+index], dtype='float32'), borrow=borrowFlag)
            ...
            featureMapsOfEachLayerAndPredictionProbabilitiesAtEndForATestBatch = \
                                cnn3dInstance.cnnTestAndVisualiseAllFmsFunction(0)
            end_training_time = time.clock()
            ...
            for imagePart_in_this_batch_i in xrange(batch_size) :
                ...
                labelImageCreatedByPredictions[
                        ...
                        coordsOfTopLeftVoxelForThisPart[0] + rczHalfRecFieldCnn[0] : coordsOfTopLeftVoxelForThisPart[0] + \
                                                        rczHalfRecFieldCnn[0] + strideOfImagePartsPerDimensionInVoxels[0],
                        coordsOfTopLeftVoxelForThisPart[1] + rczHalfRecFieldCnn[1] : coordsOfTopLeftVoxelForThisPart[1] + \
                                                        rczHalfRecFieldCnn[1] + strideOfImagePartsPerDimensionInVoxels[1],
                        coordsOfTopLeftVoxelForThisPart[2] + rczHalfRecFieldCnn[2] : coordsOfTopLeftVoxelForThisPart[2] + \
                                                        rczHalfRecFieldCnn[2] + strideOfImagePartsPerDimensionInVoxels[2],
            ...
            if saveIndividualFmImagesForVisualisation or saveMultidimensionalImageWithAllFms:
                ...
                #currentIndexInTheMultidimensionalImageWithAllToBeVisualisedFmsArray is the index in the multidimensional \
                # array that holds all the to-be-visualised-fms. It is the one that corresponds to the next \
                # to-be-visualised indexOfTheLayerInTheReturnedListByTheBatchTraining.
                currentIndexInTheMultidimensionalImageWithAllToBeVisualisedFmsArray = 0
                #indexOfTheLayerInTheReturnedListByTheBatchTraining is the index over all the layers in the returned list. \
                ...
                for pathway in cnn3dInstance.pathways :
                    for layer_i in xrange(len(pathway.getLayers())) :
                        indexOfTheLayerInTheReturnedListByTheBatchTraining += 1
                        if indicesOfFmsToVisualisePerPathwayTypeAndPerLayer[ pathway.pType() ]==[] or \
                                indicesOfFmsToVisualisePerPathwayTypeAndPerLayer[ pathway.pType() ][layer_i]==[] :
                            continue
                        indicesOfFmsToExtractFromThisLayer = \
                            indicesOfFmsToVisualisePerPathwayTypeAndPerLayer[ pathway.pType() ][layer_i]
                        
                        fmsReturnedForATestBatchForCertainLayer = \
                            listWithTheFmsOfAllLayersSortedByPathwayTypeForTheBatch[\
                                indexOfTheLayerInTheReturnedListByTheBatchTraining][:, \
                                    indicesOfFmsToExtractFromThisLayer[0]:indicesOfFmsToExtractFromThisLayer[1],:,:,:]
                        #We specify a range of fms to visualise from a layer. 
                        # currentIndexInTheMultidimensionalImageWithAllToBeVisualisedFmsArray : highIndexOfFmsInTheMultidimensionalImageToFillInThisIterationExcluding defines were to put them in the multidimensional-image-array.
                        highIndexOfFmsInTheMultidimensionalImageToFillInThisIterationExcluding = \
                            currentIndexInTheMultidimensionalImageWithAllToBeVisualisedFmsArray + \
                                indicesOfFmsToExtractFromThisLayer[1] - indicesOfFmsToExtractFromThisLayer[0]
                        fmImageInMultidimArrayToReconstructInThisIteration = \
                            multidimensionalImageWithAllToBeVisualisedFmsArray[\
                                currentIndexInTheMultidimensionalImageWithAllToBeVisualisedFmsArray: \
                                    highIndexOfFmsInTheMultidimensionalImageToFillInThisIterationExcluding]
                        ...
                        #====fmsReturnedForATestBatchForCertainLayer.shape[2] - (numberOfCentralVoxelsClassified[0]-1) \
                        ...
                        numberOfVoxToSubtrToGetPatchWidthAtThisFm_R =  numberOfCentralVoxelsClassified[0]-1 if \
                            pathway.pType() <> pt.SUBS else \
                                int(math.ceil((numberOfCentralVoxelsClassified[0]*1.0)/pathway.subsFactor()[0]) -1)
                        numberOfVoxToSubtrToGetPatchWidthAtThisFm_C =  numberOfCentralVoxelsClassified[1]-1 if \
                            pathway.pType() <> pt.SUBS else \
                                int(math.ceil((numberOfCentralVoxelsClassified[1]*1.0)/pathway.subsFactor()[1]) -1)
                        numberOfVoxToSubtrToGetPatchWidthAtThisFm_Z =  numberOfCentralVoxelsClassified[2]-1 if \
                            pathway.pType() <> pt.SUBS else \
                                int(math.ceil((numberOfCentralVoxelsClassified[2]*1.0)/pathway.subsFactor()[2]) -1)
                        rPatchDimensionAtTheFmThatWeVisualiseAfterConvolutions = \
                            fmsReturnedForATestBatchForCertainLayer.shape[2] - numberOfVoxToSubtrToGetPatchWidthAtThisFm_R
                        cPatchDimensionAtTheFmThatWeVisualiseAfterConvolutions = \
                            fmsReturnedForATestBatchForCertainLayer.shape[3] - numberOfVoxToSubtrToGetPatchWidthAtThisFm_C
                        zPatchDimensionAtTheFmThatWeVisualiseAfterConvolutions = \
                            fmsReturnedForATestBatchForCertainLayer.shape[4] - numberOfVoxToSubtrToGetPatchWidthAtThisFm_Z
                        ...
                        rOfTopLeftCentralVoxelAtTheFm = (rPatchDimensionAtTheFmThatWeVisualiseAfterConvolutions-1)/2 
                        ...
                        numberOfCentralVoxelsToGetInDirectionR = \
                            int(math.ceil((numberOfCentralVoxelsClassified[0]*1.0)/pathway.subsFactor()[0])) if \
                                pathway.pType() == pt.SUBS else numberOfCentralVoxelsClassified[0]
                        numberOfCentralVoxelsToGetInDirectionC = \
                            int(math.ceil((numberOfCentralVoxelsClassified[1]*1.0)/pathway.subsFactor()[1])) if \
                                pathway.pType() == pt.SUBS else numberOfCentralVoxelsClassified[1]
                        numberOfCentralVoxelsToGetInDirectionZ = \
                                int(math.ceil((numberOfCentralVoxelsClassified[2]*1.0)/pathway.subsFactor()[2])) if \
                                pathway.pType() == pt.SUBS else numberOfCentralVoxelsClassified[2]
                        #=============================================================================================
                        ...
                        if pathway.pType() == pt.SUBS : #subsampled layer. Remember that this returns smaller dimension \
                            ...
                            expandedOutputOfFmsR = \
                                np.repeat(centralVoxelsOfAllFmsInLayer, pathway.subsFactor()[0],axis = 2)
                        ...
                        for imagePart_in_this_batch_i in xrange(batch_size) :
                            ...
                            sliceCoordsOfThisSegment = sliceCoordsOfSegmentsInImage[imagePartOfConstructedFeatureMaps_i + \
                                                                                    imagePart_in_this_batch_i]
                            coordsOfTopLeftVoxelForThisPart = [ sliceCoordsOfThisSegment[0][0], \
                                                        sliceCoordsOfThisSegment[1][0], sliceCoordsOfThisSegment[2][0] ]
                            fmImageInMultidimArrayToReconstructInThisIteration[ # I put the central-predicted-voxels of \
                                    ...
                                    coordsOfTopLeftVoxelForThisPart[0] + rczHalfRecFieldCnn[0] : \
                                        coordsOfTopLeftVoxelForThisPart[0] + rczHalfRecFieldCnn[0] + \
                                                            strideOfImagePartsPerDimensionInVoxels[0],
                                    coordsOfTopLeftVoxelForThisPart[1] + rczHalfRecFieldCnn[1] : \
                                        coordsOfTopLeftVoxelForThisPart[1] + rczHalfRecFieldCnn[1] + \
                                                            strideOfImagePartsPerDimensionInVoxels[1],
                                    coordsOfTopLeftVoxelForThisPart[2] + rczHalfRecFieldCnn[2] : \
                                        coordsOfTopLeftVoxelForThisPart[2] + rczHalfRecFieldCnn[2] + \
                                                            strideOfImagePartsPerDimensionInVoxels[2]
                                    ] = centralVoxelsOfAllFmsToBeVisualisedForWholeBatch[imagePart_in_this_batch_i]
                        currentIndexInTheMultidimensionalImageWithAllToBeVisualisedFmsArray = \
                            highIndexOfFmsInTheMultidimensionalImageToFillInThisIterationExcluding
                imagePartOfConstructedFeatureMaps_i += batch_size #all the image parts before this were reconstructed \
        ...
        if savePredictionImagesSegmentationAndProbMapsList[0] == True : #save predicted segmentation
            ...
            unpaddedSegmentationImage = segmentationImage if not padInputImagesBool else \
                unpadCnnOutputs(segmentationImage, tupleOfPaddingPerAxesLeftRight)
        ...
        for class_i in xrange(0, NUMBER_OF_CLASSES) :
            if (len(savePredictionImagesSegmentationAndProbMapsList[1]) >= class_i + 1) and \
                (savePredictionImagesSegmentationAndProbMapsList[1][class_i] == True) : #save predicted probMap for class
                ...
                unpaddedPredictedLabelImageForSpecificClass = predictedLabelImageForSpecificClass if not \
                padInputImagesBool else unpadCnnOutputs(predictedLabelImageForSpecificClass, tupleOfPaddingPerAxesLeftRight)
                ...
                if indicesOfFmsToVisualisePerLayerOfCertainPathway<>[] :
                    for layer_i in xrange( len(pathway.getLayers()) ) :
                        ...
                        if indicesOfFmsToVisualiseForCertainLayerOfCertainPathway<>[] :
                            #If the user specifies to grab more feature maps than exist (eg 9999), correct it, \
                            # replacing it with the number of FMs in the layer.
                            for fmActualNumber in xrange(indicesOfFmsToVisualiseForCertainLayerOfCertainPathway[0], \
                                                         indicesOfFmsToVisualiseForCertainLayerOfCertainPathway[1]) :
                                fmToSave = multidimensionalImageWithAllToBeVisualisedFmsArray[\
                                                    currentIndexInTheMultidimensionalImageWithAllToBeVisualisedFmsArray]
                                unpaddedFmToSave = fmToSave if not padInputImagesBool else \
        ...
        if saveMultidimensionalImageWithAllFms :
            """
            multidimensionalImageWithAllToBeVisualisedFmsArrayWith4thDimAsFms =  \
                np.zeros(niiDimensions + [totalNumberOfFMsToProcess], dtype = "float32")
            for fm_i in xrange(0, totalNumberOfFMsToProcess) :
                multidimensionalImageWithAllToBeVisualisedFmsArrayWith4thDimAsFms[:,:,:,fm_i] = \
                multidimensionalImageWithAllToBeVisualisedFmsArray[fm_i]
            """
            multidimensionalImageWithAllToBeVisualisedFmsArrayWith4thDimAsFms =  \
                np.transpose(multidimensionalImageWithAllToBeVisualisedFmsArray, (1,2,3, 0) )
            
            unpaddedMultidimensionalImageWithAllToBeVisualisedFmsArrayWith4thDimAsFms = \
                multidimensionalImageWithAllToBeVisualisedFmsArrayWith4thDimAsFms if not padInputImagesBool else \
            ...
            saveMultidimensionalImageWithAllVisualisedFmsToANewNiiWithHeaderFromOther( \
                                                unpaddedMultidimensionalImageWithAllToBeVisualisedFmsArrayWith4thDimAsFms,
                                                listOfNamesToGiveToFmVisualisationsIfSaving,
                                                listOfFilepathsToEachChannelOfEachPatient,
                                                image_i,
        ...
        if providedGtLabelsBool : # Ground Truth was provided for calculation of DSC. Do DSC calculation.
            ...
            unpaddedSegmentationImage = segmentationImage if not padInputImagesBool else \
                unpadCnnOutputs(segmentationImage, tupleOfPaddingPerAxesLeftRight)
            unpaddedGtLabelsImage = gtLabelsImage if not padInputImagesBool else \
            ...
            if isinstance(brainMask, (np.ndarray)) : #If brainmask was given:
                multiplyWithBrainMaskOr1 = brainMask if not padInputImagesBool else \
                unpadCnnOutputs(brainMask, tupleOfPaddingPerAxesLeftRight)
            else :
            ...
            for class_i in xrange(0, NUMBER_OF_CLASSES) :
                ...
                diceCoeff1 = calculateDiceCoefficient(booleanPredictedLabelImage, \
                                                      booleanGtLesionLabelsForDiceEvaluation_unstripped)
                diceCoeffs1[image_i][class_i] = diceCoeff1 if diceCoeff1 <> -1 else NA_PATTERN
                #Dice2 = PredictedWithinBrainMask / AllLesions
                diceCoeff2 = calculateDiceCoefficient(predictedLabelImageConvolvedWithBrainMask, \
                                                      booleanGtLesionLabelsForDiceEvaluation_unstripped)
                diceCoeffs2[image_i][class_i] = diceCoeff2 if diceCoeff2 <> -1 else NA_PATTERN
                #Dice3 = PredictedWithinBrainMask / LesionsInsideBrainMask
                diceCoeff3 = calculateDiceCoefficient(predictedLabelImageConvolvedWithBrainMask, \
                                                      booleanGtLesionLabelsForDiceEvaluation_unstripped * \
                                                      multiplyWithBrainMaskOr1)
                diceCoeffs3[image_i][class_i] = diceCoeff3 if diceCoeff3 <> -1 else NA_PATTERN
                
            myLogger.print3("ACCURACY: (" + str(validationOrTestingString) + \
    ...
    if providedGtLabelsBool and total_number_of_images>0 : 
    ...
    myLogger.print3("TIMING: "+validationOrTestingString+" process took time: "+\
                    str(end_validationOrTesting_time-start_validationOrTesting_time)+"(s)")
    
#     myLogger.print3("###########################################################################################################")
    myLogger.print3("############################# Finished full Segmentation of " + str(validationOrTestingString) + \
                    " subjects ##########################")
#     myLogger.print3("###########################################################################################################")
