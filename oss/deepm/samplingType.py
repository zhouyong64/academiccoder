...
class SamplingType(object) :
    def __init__(self, myLogger, samplingType, numberOfClassesInclBackgr):
        ...
        if self.samplingType == 0 :
            ...
            self.logger.print3("ERROR: Tried to create a SamplingType instance, but the samplingType passed was invalid. \
            Should be [0,1,2,3]. Exiting!"); exit(1)
            
    def setPercentOfSamplesPerCategoryToSample(self, percentageOfSamplesPerCategoryOfSampling) :
        if self.samplingType in [0,3] and \
            len(percentageOfSamplesPerCategoryOfSampling) <> self.getNumberOfCategoriesToSample() :
            self.logger.print3("ERROR: In class [SamplingType], the list percentageOfSamplesPerCategoryOfSampling had [" + \
                               str(len(percentageOfSamplesPerCategoryOfSampling)) + "] elements. In this case of [" + \
                               self.stringOfSamplingType + "], it should have [" + \
                               str(self.getNumberOfCategoriesToSample()) + "]! Exiting!"); exit(1)
        ...
        if self.samplingType == 0 :
            ...
            self.logger.print3("ERROR: in [SamplingType]. self.samplingType was invalid. Should be [0,1,2,3]. Exiting!"); 
            exit(1)
    ...
    def logicDecidingAndGivingFinalSamplingMapsForEachCategory( self,
                                                                ...
                                                                providedGtLabelsBool, #Always true both for training and \
                                                                # validation. So, I can take this out.
                                                                gtLabelsImage,
        ...
        if self.samplingType == 0 : # fore/background
            if providedWeightMapsToSampleForEachCategory : #Both weight maps should be provided currently.
                ...
                if numOfProvidedWeightMaps <> self.getNumberOfCategoriesToSample() :
                    self.logger.print3("ERROR: For SamplingType = Fore/Background(0), [" + str(numOfProvidedWeightMaps) + \
                                       "] weight maps were provided! Two (fore/back) were expected! Exiting!"); exit(1)        
                ...
                self.logger.print3("ERROR: For SamplingType=[" + self.stringOfSamplingType + \
                                   "], if weighted-maps are not provided, at least Ground Truth labels \
                                   should be given to extract foreground! Exiting!"); exit(1)
                ...
                #Foreground / Background (in sequence)
                finalWeightMapsToSampleFromPerCategoryForSubject = [ maskForForegroundSampling, \
                                                                    maskForBackgroundSampling_roiMinusGtLabels ] 
            else : # no weightmaps, gt provided and roi is not provided.
                maskForForegroundSampling = (gtLabelsImage>0).astype(int)
                maskForBackgroundSampling_roiMinusGtLabels = np.ones(dimensionsOfImageChannel, dtype="int16") * \
                                                                                (maskForForegroundSampling==0)
                #Foreground / Background (in sequence)
                finalWeightMapsToSampleFromPerCategoryForSubject = [ maskForForegroundSampling, \
                                                                    maskForBackgroundSampling_roiMinusGtLabels ] 
            ...
            if providedWeightMapsToSampleForEachCategory :
                ...
                if numOfProvidedWeightMaps <> 1 :
                    self.logger.print3("ERROR: For SamplingType=[" + self.stringOfSamplingType + "], [" + \
                            str(numOfProvidedWeightMaps) + "] weight maps were provided! One was expected! Exiting!"); 
                    exit(1)
                #Should be an array with dim1==1 already.
                finalWeightMapsToSampleFromPerCategoryForSubject = arrayWithWeightMapsWhereToSampleForEachCategory 
            elif providedRoiMaskBool :
                #Be careful to not change either of the two arrays later or there'll be a problem.
                finalWeightMapsToSampleFromPerCategoryForSubject = [ roiMask ] 
            else :
            ...
            if providedWeightMapsToSampleForEachCategory :
                ...
                if numOfProvidedWeightMaps <> 1 :
                    self.logger.print3("ERROR: For SamplingType=[" + self.stringOfSamplingType + "], [" + \
                                       str(numOfProvidedWeightMaps) + \
                                       "] weight maps were provided! One was expected! Exiting!"); 
                    exit(1)
                #Should be an array with dim1==1 already.
                finalWeightMapsToSampleFromPerCategoryForSubject = arrayWithWeightMapsWhereToSampleForEachCategory 
            elif providedRoiMaskBool :
            ...
            if providedWeightMapsToSampleForEachCategory :
                ...
                if numOfProvidedWeightMaps <> self.getNumberOfCategoriesToSample() :
                    self.logger.print3("ERROR: For SamplingType=[" + self.stringOfSamplingType + "], [" + \
                                       str(numOfProvidedWeightMaps) + "] weight maps were provided! As many as the classes [" + \
                                       str(self.getNumberOfCategoriesToSample()) + "] (incl Background) were expected! Exiting!"); 
                    exit(1)
                #Should have as many entries as classes (incl backgr).
                finalWeightMapsToSampleFromPerCategoryForSubject = arrayWithWeightMapsWhereToSampleForEachCategory 
            elif providedGtLabelsBool :
                finalWeightMapsToSampleFromPerCategoryForSubject = []
                 # Should be the same number as the number of actual classes, including background.
                for cat_i in xrange( self.getNumberOfCategoriesToSample() ) :
                    finalWeightMapsToSampleFromPerCategoryForSubject.append( (gtLabelsImage == cat_i).astype(int) )
            else :
                self.logger.print3("ERROR: For SamplingType=TargettedPerClass(3), either weightMaps for each class \
                    or GT labels should be given! Exiting!"); exit(1)
        else :
            self.logger.print3("ERROR: Sampling-type-number passed in [logicDecidingAndGivingFinalSamplingMapsForEachCategory] \
                was invalid. Should be [0,1,2,3]. Exiting!"); exit(1)
