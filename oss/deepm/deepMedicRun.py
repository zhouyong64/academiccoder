...
def setupArgParser() :
    ...
    description="\nThis software allows creation and supervised training of 3D, multi-scale CNN models for segmentation \
    ...
    parser.add_argument("-newModel", dest='model_conf', type=str, help="Create a new CNN model with model parameters \
    																			at given config file [MODEL_CONF].")
    parser.add_argument("-train", dest='training_conf', type=str, help="Train a CNN model with training parameters at \
    																			given config file [TRAINING_CONF].\n"+\
    ...
    parser.add_argument("-test", dest='testing_conf', type=str, help="Test with an existing CNN model. \
    	The testing session's parameters should be given in config file [TESTING_CONF].\n"+\
    ...
    parser.add_argument("-dev", default = "cpu", dest='device', type=str,  \
    	help="Specify the device to run the process on. Values: [cpu] or [gpu] (default = cpu).\n"+\
    ...
    parser.add_argument("-pretrained", dest='pretrained_model', type=str, \
    				help="Use to transfer the weights from a previously trained model to a new model.\n"+\
      ...
      "NOTE: By default, parameters are transfered to all layers except the classification layer. \
      Use option [-layers] to manually specify layers to pretrain.")
    parser.add_argument("-layers", dest='layers_to_transfer', nargs='+', type=int, \
    		help="Use only after a [-pretrained] option.\n"+\
                                    "Specify to which layers of the new model parameters should be transferred to.\n"+\
     "First layer is 1. Classification layer of original DeepMedic is 11. \
     	Same layers from each parallel-pathway will be transfered.\n"+\
        "Usage: ./deepMedicRun -newModel /path/to/model/config -pretrained /path/to/pretrained/model -layers 1 2 3 ...")
    parser.add_argument("-resetOptimizer", dest='reset_optimizer', action='store_true', \
    	help="Use optionally with a [-train] command. Does not take an argument.\n"+\
             ...
             "Useful to begin a secondary training session with new learning-rate schedule, \
             	in order to fine-tune a previously trained model (Doc., Sec. 3.2)")
    
    return parser

def checkDevicePassedCorrectly(devArg) :
    if devArg <> "cpu" and devArg <> "gpu" and not (devArg.startswith("gpu") and strIsInt(devArg[3:]) ) :
        print("ERROR: Value for the [-dev] option was not specified correctly. Specify the device to run the process on. \
        Values: [cpu] or [gpu] (Default = cpu).\nIn the case of multiple GPUs, specify a particular GPU device with a number, \
        in the format: gpu2. Exiting."); exit(1)
...
def setEnvironment(deviceString) :
    ...
    os.environ['THEANO_FLAGS']='on_unused_input=warn, mode=FAST_RUN,device=' + deviceString + \
...
if __name__ == '__main__':
    ...
    if args.model_conf and args.saved_model :
        ...
        Use [-newModel] to create a new cnn model.\n\t[-model] can follow a [-train] or [-test] option, \
        to specify which saved model to use.\n\tPlease try -h for more information. Exiting."); exit(1)
    if args.testing_conf and (args.model_conf or args.training_conf) :
        print("ERROR: [-test] cannot be used in conjuction with [-newModel] or [-train]. \
        To test with an existing network, please just specify a configuration file for the testing process, \
        which will include a path to a trained model to use for the inference. Exiting."); exit(1)
