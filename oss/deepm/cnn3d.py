...
class Cnn3d(object):
    ...
    def make_cnn_model( self,
        ...
        self.recFieldCnn = \
            calculateReceptiveFieldDimensionsFromKernelsDimListPerLayerForFullyConvCnnWithStrides1(kernelDimensions)
        ...
        thisPathwayUseBnPerLayer[0] = applyBnToInputOfPathways[thisPathwayType] if \
            rollingAverageForBatchNormalizationOverThatManyBatches > 0 else False  # For the 1st layer, ask specific flag.
        ...
        thisPathwayActivFuncPerLayer[0] = -1 if thisPathwayType <> pt.FC else activationFunctionToUseRelu0orPrelu1  
        ...
        thisPathway.makeMultiscaleConnectionsForLayerType(\
                                        convLayersToConnectToFirstFcForMultiscaleFromAllLayerTypes[thisPathwayType])
        ...
        for subPath_i in xrange(self.numSubsPaths) :
            ...
            thisPathwayActivFuncPerLayer = [activationFunctionToUseRelu0orPrelu1] * thisPathwayNumOfLayers
            thisPathwayActivFuncPerLayer[0] = -1 if thisPathwayType <> pt.FC else \
                 activationFunctionToUseRelu0orPrelu1  # To not apply activation on raw input. -1 is linear activation.
            
            inputToPathwayShapeTrain = [self.batchSize, numberOfImageChannelsPath2] + \
            thisPathway.calcInputRczDimsToProduceOutputFmsOfCompatibleDims(thisPathWayKernelDimensions, \
                                                                           dimsOfOutputFrom1stPathwayTrain);
            inputToPathwayShapeVal = [self.batchSizeValidation, numberOfImageChannelsPath2] + \
                thisPathway.calcInputRczDimsToProduceOutputFmsOfCompatibleDims(thisPathWayKernelDimensions, \
                                                                               dimsOfOutputFrom1stPathwayVal)
