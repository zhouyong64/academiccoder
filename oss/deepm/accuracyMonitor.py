...
class AccuracyOfEpochMonitorSegmentation(object) :
    ...
    def __init__(self,
        ...
        # --- mean Empirical Accuracy: number of Corr Classified voxels and all voxels, 
        # wrt to all the classes, in multiclass problems.---
        ...
        # mean (multiclass) accuracy. correctlyPredVoxelsInEachSubep / numberOfAllSamplesOfEachSubep 
        # NOTE: Cannot contain N0T-APPLICABLE
        ...
        # NOTE: May contain N0T-APPLICABLE=self.NA_PATTERN elements, eg when class not present!
    ...
    # ==== API ====
    # Multiclass mean accuracy. As in: Number-of-Voxels-Correctly-Classified / All-voxels
    ...
    def updateMonitorAccuraciesWithNewSubepochEntries(  self,
                                                        meanCostOfSubepoch,
                                                        perClassRpRnTpTnInSubep # Class X 4. The Real Pos, Real Neg, 
                                                        #True Pos (pred), True Neg (pred).
                                                        ) :
        #---------- This first part takes care of the overall, multi-class mean accuracy: 
        #meanAccuracy = Num-Voxels-Predicted-Correct-Class / All-Voxels. 
        if self.training0orValidation1 == 0 :
        ...
        for class_i in xrange(self.numberOfClasses) :
            # Add up the true positives per class (which are one vs all).
        ...
        #RealPos + RealNeg wrt any class (eg backgr)
        ...
        meanAccuracyOfSubepoch = self.NA_PATTERN if numberOfAllSamples == 0 \
            else correctlyPredVoxelsInSubep*1.0/numberOfAllSamples
        ...
        for class_i in xrange(self.numberOfClasses) :
            ...
            meanAccuracyClassVsAllOfSubep = \
                (numOfTruePosInSubep+numOfTrueNegInSubep) / float(numOfRealPosInSubep+numOfRealNegInSubep)
            meanAccuracyOnPosOfSubep = self.NA_PATTERN if numOfRealPosInSubep == 0 else \
                numOfTruePosInSubep*1.0/numOfRealPosInSubep
            meanAccuracyOnNegOfSubep = self.NA_PATTERN if numOfRealNegInSubep == 0 else \
                numOfTrueNegInSubep*1.0/numOfRealNegInSubep
            ...
            meanDiceOfSubep = self.NA_PATTERN if numOfRealPosInSubep == 0 else \
                (2.0*numOfTruePosInSubep)/(numOfPredPosInSubep + numOfRealPosInSubep)
            
            listWithPerClassMeanAccSensSpecDscInSubep.append( [meanAccuracyClassVsAllOfSubep, \
                                                               meanAccuracyOnPosOfSubep, \
                                                               meanAccuracyOnNegOfSubep, meanDiceOfSubep] )
        self.listPerSubepPerClassMeanAccSensSpecDsc.append(listWithPerClassMeanAccSensSpecDscInSubep)
        
        # ===============UPDATE THE MERGED FOREGROUND CLASS. 
        #(used in multi-class problems instead of class-0 background).================
        # RealNeg/Pos of background is the RealPos/Neg of the foreground. 
        #TrueNeg/Pos of background is the TruePos/Neg of the foreground.
        foregrTp = perClassRpRnTpTnInSubep[0][3]; foregrTn = perClassRpRnTpTnInSubep[0][2]; 
        foregrRp = perClassRpRnTpTnInSubep[0][1]; foregrRn = perClassRpRnTpTnInSubep[0][0]
        ...
        self.listPerSubepForegrMeanAccSensSpecDsc.append( [ foregrMeanAccOfSubep, foregrMeanAccOnPosOfSubep, \
                                                           foregrMeanAccOnNegOfSubep, foregrMeanDiceOfSubep] )
    ...
    def reportAccuracyForLastSubepoch(self) :
        ...
        self.logger.print3( logStr + ", Overall:\t mean accuracy:   \t" + \
                            strFl4fNA(self.meanEmpiricalAccuracyOfEachSubep[currSubep], self.NA_PATTERN) +\
                        "\t=> Correctly-Classified-Voxels/All-Predicted-Voxels = " + \
                        str(self.correctlyPredVoxelsInEachSubep[currSubep]) + "/" + \
                        str(self.numberOfAllSamplesOfEachSubep[currSubep]) )
        if self.training0orValidation1 == 0 : # During training, also report the mean value of the Cost Function:
            self.logger.print3( logStr + ", Overall:\t mean cost:      \t" + \
                                strFl5fNA(self.meanCostOfEachSubep[currSubep], self.NA_PATTERN) ) 
            
        # Report accuracy over subepoch for each class_i:
        for class_i in xrange(self.numberOfClasses) :
            classString = "Class-"+str(class_i)
            extraDescription = "[Whole Foreground (Pos) Vs Background (Neg)]" if class_i == 0 else \
                "[This Class (Pos) Vs All Others (Neg)]"
            
            self.logger.print3( "+++++++++++++++ Reporting Accuracy over whole subepoch for " + classString + \
                                " ++++++++ " + extraDescription + " ++++++++++++++++" )
            ...
            self.logger.print3(logStrClass+"\t mean specificity:\t"+ strFl4fNA(meanAccOnNegOfSubep, self.NA_PATTERN)+\
                               "\t=> TrueNeg/RealNeg = "+str(numOfTnInSubep)+"/"+str(numOfRnInSubep))
    ...
    def reportMeanAccyracyOfEpoch(self) :
        ...
        self.logger.print3( logStr + ", Overall:\t mean accuracy of epoch:\t" + \
                            strFl4fNA(meanEmpiricalAccOfEp, self.NA_PATTERN)+\
                            "\t=> Correctly-Classified-Voxels/All-Predicted-Voxels")
        if self.training0orValidation1 == 0 : # During training, also report the mean value of the Cost Function:
            meanCostOfEp = getMeanOfListExclNA(self.meanCostOfEachSubep, self.NA_PATTERN)
            self.logger.print3( logStr + ", Overall:\t mean cost of epoch:    \t" + \
                                strFl5fNA(meanCostOfEp, self.NA_PATTERN) )
            
        self.logger.print3( logStr + ", Overall:\t mean accuracy of each subepoch:\t"+ \
                            strListFl4fNA(self.meanEmpiricalAccuracyOfEachSubep, self.NA_PATTERN) )
        if self.training0orValidation1 == 0 :
            self.logger.print3( logStr + ", Overall:\t mean cost of each subepoch:    \t" + \
        ...
        for class_i in xrange(self.numberOfClasses) :
            ...
            self.logger.print3( ">>>>>>>>>>>> Reporting Accuracy over whole epoch for " + classString + \
                                " >>>>>>>>> " + extraDescription + " <<<<<<<<<<<<<" )
            
            if class_i <> 0 :
                meanAccPerSubep = [ self.listPerSubepPerClassMeanAccSensSpecDsc[subep_i][class_i][0] for subep_i \
                                   in xrange(len(self.listPerSubepPerClassMeanAccSensSpecDsc)) ]
                meanSensPerSubep = [ self.listPerSubepPerClassMeanAccSensSpecDsc[subep_i][class_i][1] for subep_i \
                                    in xrange(len(self.listPerSubepPerClassMeanAccSensSpecDsc)) ]
                meanSpecPerSubep = [ self.listPerSubepPerClassMeanAccSensSpecDsc[subep_i][class_i][2] for subep_i \
                                    in xrange(len(self.listPerSubepPerClassMeanAccSensSpecDsc)) ]
                meanDscPerSubep = [ self.listPerSubepPerClassMeanAccSensSpecDsc[subep_i][class_i][3] for subep_i \
                                   in xrange(len(self.listPerSubepPerClassMeanAccSensSpecDsc)) ]
            else : # Foreground Vs Background
                meanAccPerSubep = [ self.listPerSubepForegrMeanAccSensSpecDsc[subep_i][0] for subep_i \
                                   in xrange(len(self.listPerSubepForegrMeanAccSensSpecDsc)) ]
                meanSensPerSubep = [ self.listPerSubepForegrMeanAccSensSpecDsc[subep_i][1] for subep_i \
                                    in xrange(len(self.listPerSubepForegrMeanAccSensSpecDsc)) ]
                meanSpecPerSubep = [ self.listPerSubepForegrMeanAccSensSpecDsc[subep_i][2] for subep_i \
                                    in xrange(len(self.listPerSubepForegrMeanAccSensSpecDsc)) ]
                meanDscPerSubep = [ self.listPerSubepForegrMeanAccSensSpecDsc[subep_i][3] for subep_i \
                                   in xrange(len(self.listPerSubepForegrMeanAccSensSpecDsc)) ]
            ...
            logStrClass = logStr + ", " + classString + ":"
            self.logger.print3(logStrClass + "\t mean accuracy of epoch:\t"+ strFl4fNA(meanAccOfEp, self.NA_PATTERN) +\
                               "\t=> (TruePos+TrueNeg)/All-Predicted-Voxels")
            self.logger.print3(logStrClass + "\t mean sensitivity of epoch:\t"+ strFl4fNA(meanSensOfEp, self.NA_PATTERN) \
                               +"\t=> TruePos/RealPos")
            self.logger.print3(logStrClass + "\t mean specificity of epoch:\t"+ strFl4fNA(meanSpecOfEp, self.NA_PATTERN) \
                               +"\t=> TrueNeg/RealNeg")
            self.logger.print3(logStrClass + "\t mean Dice of epoch:    \t"+ strFl4fNA(meanDscOfEp, self.NA_PATTERN) )
            
            #Visualised in my scripts:
            self.logger.print3(logStrClass + "\t mean accuracy of each subepoch:\t"+ \
                               strListFl4fNA(meanAccPerSubep, self.NA_PATTERN) )
            self.logger.print3(logStrClass + "\t mean sensitivity of each subepoch:\t" + \
                               strListFl4fNA(meanSensPerSubep, self.NA_PATTERN) )
            self.logger.print3(logStrClass + "\t mean specificity of each subepoch:\t" + \
                               strListFl4fNA(meanSpecPerSubep, self.NA_PATTERN) )
            self.logger.print3(logStrClass + "\t mean Dice of each subepoch:    \t" + \
