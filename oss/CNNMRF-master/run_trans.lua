...
-- max_size: maximum size of the synthesis image. Default value 384. 
   --Larger image needs more time and memory.
...
-- mrf_weight: weight for each MRF layer. Default value 1e-4. 
  --Higher weights leads to more style faithful results.
...
-- mrf_sample_stride: stride to sample mrf on style image. Default value 2. 
   --This value is defined seperately for each MRF layer.
-- mrf_synthesis_stride: stride to sample mrf on synthesis image. 
   --Default value 2. This value is defined seperately for each MRF layer.
-- mrf_confidence_threshold: threshold for filtering out bad matching. Default value 0 
   -- means we keep all matchings. This value is defined seperately for all layers.

-- content_layers: the layers for content constraint. Default value 23.
-- content_weight: The weight for content constraint. Default value 2e1. Increasing this value will make 
   --the result more content faithful. Decreasing the value will make the method more style faithful. 
   --Notice this value should be increase (for example, doubled) if layer 12 is included for MRF constraint,  
