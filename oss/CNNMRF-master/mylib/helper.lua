function computeMRF(input, size, stride, gpu, backend)
	...
	for i_row = 1, dim_2 do
	  for i_col = 1, dim_3 do
	    t_feature_mrf[count] = input[{{1, input:size()[1]}, 
    {coord_y[i_row], coord_y[i_row] + size - 1}, {coord_x[i_col], coord_x[i_col] + size - 1}}]
	    count = count + 1
	  end
	end
...
function sampleMRFAndTensorfromLocation2(coord_x, coord_y, input, size, gpu)
	...
	for i_patch = 1, coord_x:nElement() do
		t_feature_mrf[i_patch] = input[{{1, input:size()[1]}, 
    {coord_y[i_patch], coord_y[i_patch] + size - 1}, {coord_x[i_patch], coord_x[i_patch] + size - 1}}]
	end
...
function computeBB(width, height, alpha)
	...
	if alpha > 0 then
	  ...
	  local px1 = ((x1 * y4 - y1 * x4) * (x1r - x2r) - 
    (x1 - x4) * (x1r * y2r - y1r * x2r)) / ((x1 - x4) * (y1r - y2r) - (y1 - y4) * (x1r - x2r))
	  local py1 = ((x1 * y4 - y1 * x4) * (y1r - y2r) - 
    (y1 - y4) * (x1r * y2r - y1r * x2r)) / ((x1 - x4) * (y1r - y2r) - (y1 - y4) * (x1r - x2r))
	  local px2 = px1 + 1
	  local py2 = py1
	  -- print(px1 .. ' ' .. py1)
	  -- find the intersection Q of line [px1, py1]-[px2, py2] and [x2r, y2r]-[x3r][y3r]

	  local qx = ((px1 * py2 - py1 * px2) * (x2r - x3r) - 
    (px1 - px2) * (x2r * y3r - y2r * x3r)) / ((px1 - px2) * (y2r - y3r) - (py1 - py2) * (x2r - x3r))
	  local qy = ((px1 * py2 - py1 * px2) * (y2r - y3r) - 
    (py1 - py2) * (x2r * y3r - y2r * x3r)) / ((px1 - px2) * (y2r - y3r) - (py1 - py2) * (x2r - x3r))  
	  -- print(qx .. ' ' .. qy)
	  ...
	  local px1 = ((x2 * y3 - y2 * x3) * (x1r - x2r) - 
            (x2 - x3) * (x1r * y2r - y1r * x2r)) / ((x2 - x3) * (y1r - y2r) - (y2 - y3) * (x1r - x2r))
	  local py1 = ((x2 * y3 - y1 * x3) * (y1r - y2r) - 
            (y2 - y3) * (x1r * y2r - y1r * x2r)) / ((x2 - x3) * (y1r - y2r) - (y2 - y3) * (x1r - x2r))
	  local px2 = px1 - 1
	  local py2 = py1
	  -- find the intersection Q of line [px1, py1]-[px2, py2] and [x1r, y1r]-[x4r][y4r]
	  local qx = ((px1 * py2 - py1 * px2) * (x1r - x4r) - 
            (px1 - px2) * (x1r * y4r - y1r * x4r)) / ((px1 - px2) * (y1r - y4r) - (py1 - py2) * (x1r - x4r))
	  local qy = ((px1 * py2 - py1 * px2) * (y1r - y4r) - 
            (py1 - py2) * (x1r * y4r - y1r * x4r)) / ((px1 - px2) * (y1r - y4r) - (py1 - py2) * (x1r - x4r))  
	  min_x = qx
	  min_y = qy
	  max_x = width - min_x
	  max_y = height - min_y
